...
This package is a current-v3p5 BGO equal-attenuation geometry sample for TES_511_Balloon. The Step01 geometry/control closure is complete, and a small Step02 prompt/buildup smoke transport has been run. It is still not a BGO background-rate or sensitivity result.
...
| Cosima overlap | PASS |
| Step02 prompt/buildup smoke | PASS_BGO_SAMPLE_STEP02_SMOKE_TRANSPORT |
...
- Step02 smoke has run only `gamma,p` with 500 gamma-equivalent events for prompt and activation-build-up transport connectivity.
- Full all-particle Step02 production, delayed-source construction, delayed transport, Step05 veto/time-axis response, Step08 significance, and detector-coupled Step09 response remain `NOT_RUN` for this sample.
