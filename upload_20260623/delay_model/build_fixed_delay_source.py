...
def _build_normalization_audit(
    ...
    file_count_by_tag = Counter(parse_tag(path) for path in dat_files)
    div_by_tag: dict[str, float] = {}
    audit: dict[str, dict[str, object]] = {}
    for tag in sorted(file_count_by_tag):
        files = int(file_count_by_tag[tag])
        div = _resolve_div(tag, files, non_gamma_div, gamma_div)
        div_by_tag[tag] = div
        audit[tag] = {
            "tag": tag,
            "files": files,
...
def _finalize_normalization_audit(
    audit: dict[str, dict[str, object]],
    allow_div_mismatch: bool,
) -> list[str]:
    problems: list[str] = []
    for tag, row in audit.items():
        ...
        if tt_min is not None and tt_max is not None and tt_mean > 0.0:
...
def _write_normalization_audit(
    ...
    for tag in sorted(audit):
        ...
        rows.append(row)
    csv_path = outdir / "normalization_audit_groundstate_fix.csv"
    json_path = outdir / "normalization_audit_groundstate_fix.json"
    fields = [
        "tag",
        "files",
        "division",
...
def parse_rp_from_dat(
    dat_files: list[Path],
    non_gamma_div: float,
    gamma_div: str,
    allow_div_mismatch: bool,
) -> tuple[dict[str, float], dict[tuple[str, str, int, float], float], dict[str, dict[str, object]], list[str]]:
    ...
    for path in dat_files:
        ...
        with path.open("r", encoding="utf-8", errors="ignore") as fh:
            for line in fh:
                ...
                if m and cur_vn is not None:
                    ...
                    scaled_val = raw_val / div
                    rp[(tag, cur_vn, za, exc)] += scaled_val
                    audit[tag]["rp_raw_total"] = float(audit[tag]["rp_raw_total"]) + raw_val
                    audit[tag]["rp_scaled_total"] = float(audit[tag]["rp_scaled_total"]) + scaled_val
        ...
        if tt_vals_in_file:
            ...
            if len(tt_vals_in_file) > 1:
                audit[tag]["warnings"].append(f"{path.name} has {len(tt_vals_in_file)} TT lines")
            tt_val = tt_vals_in_file[-1]
            if tt_val > 0.0:
                tt_values_by_tag[tag].append(tt_val)
    for tag, values in tt_values_by_tag.items():
        row = audit[tag]
        row["tt_count"] = len(values)
        row["tt_files"] = int(row["tt_files"])
        row["tt_sum_s"] = sum(values)
        row["tt_mean_s"] = sum(values) / len(values)
        row["tt_min_s"] = min(values)
        row["tt_max_s"] = max(values)
...
def main() -> None:
    ...
    ap.add_argument("--gamma-div", default="auto", help="Gamma tag normalization divisor, or 'auto' for gamma file count.")
    ap.add_argument("--allow-div-mismatch", action="store_true", help="Allow tag file-count/divisor mismatches after writing the audit.")
    ...
    tt_by_tag, rp, normalization_audit, normalization_problems = parse_rp_from_dat(
        dat_files,
        args.non_gamma_div,
        args.gamma_div,
        args.allow_div_mismatch,
    )
    normalization_audit_csv, normalization_audit_json = _write_normalization_audit(
        args.outdir,
        normalization_audit,
    ...
    if normalization_problems:
        raise SystemExit(
            "Normalization audit failed; inspect "
            f"{normalization_audit_json}: " + "; ".join(normalization_problems)
        )
    ...
    summary = {
        ...
        "normalization_audit_csv": str(normalization_audit_csv),
        "normalization_audit_json": str(normalization_audit_json),
    ...
    print(f"[OK] wrote {normalization_audit_csv}")
    print(f"[OK] wrote {normalization_audit_json}")
