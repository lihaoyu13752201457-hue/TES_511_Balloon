#!/usr/bin/env python3
"""Summarize the v3p5 center-finger low-stat Step02 closure."""
...
OUT = STEP / "outputs_v3p5_centerfinger_lowstat10k"
SNAP = STEP / "source_snapshots_v3p5_centerfinger_lowstat10k"
...
INSTANT = ROOT / "runs" / "step02_instant_v3p5_centerfinger_lowstat10k"
BUILDUP = ROOT / "runs" / "step02_buildup_v3p5_centerfinger_lowstat10k"
DECAY = ROOT / "runs" / "step02_decay_source_v3p5_centerfinger_lowstat10k"
FIX = ROOT / "runs" / "step02_delay_fix_v3p5_centerfinger_lowstat10k"
DELAYED = ROOT / "runs" / "step02_delayed_transport_v3p5_centerfinger_lowstat10k"

SUMMARY_JSON = OUT / "step02_v3p5_centerfinger_lowstat10k_summary.json"
SUMMARY_MD = OUT / "step02_v3p5_centerfinger_lowstat10k_summary.md"
PARTICLE_COUNTS = OUT / "step02_v3p5_centerfinger_lowstat10k_particle_counts.csv"
SOURCE_BLOCKS = OUT / "step02_v3p5_centerfinger_lowstat10k_source_blocks.csv"
...
def write_snapshots() -> None:
    ...
    for src, dst in [
        ...
        (DELAYED / "cosima_delayed_transport_lowstat10k.log", "cosima_delayed_transport_lowstat10k.log"),
...
def markdown(summary: dict[str, Any], particle_rows: list[dict[str, Any]]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- delayed transport log: `{rel(DELAYED / 'cosima_delayed_transport_lowstat10k.log')}`",
            "",
...
def main() -> int:
    ...
    payload = {
        "status": "PASS_LOWSTAT10K_TRANSPORT_CLOSURE",
        "scope": "all-particle low-stat smoke closure; not final 1/10 or full-stat physics result",
        "geometry": geom,
        ...
        "known_limitations": [
            "low-stat run uses gamma_events=10000, not the requested final 1/10-statistics production",
