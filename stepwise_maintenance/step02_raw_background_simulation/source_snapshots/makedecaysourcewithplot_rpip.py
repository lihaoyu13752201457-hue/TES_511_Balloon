...
def parse_rp_from_dat(dat_files: list[str], non_gamma_div: float):
    """Return:
    - tt_by_tag[tag] = TT seconds (if present)
    ...
    for fp in dat_files:
        tag = parse_tag(fp)
        div = 1.0 if is_gamma(tag) else float(non_gamma_div)
        if div <= 0:
            div = 1.0
        ...
        if tt_val is not None and tt_val > 0:
...
def _parse_one_sim_for_rpip(fp: str, non_gamma_div: float, want_proc: str) -> RPIPFileResult:
    tag = parse_tag(fp)
    wfile = 1.0 if is_gamma(tag) else (1.0 / float(non_gamma_div))
...
def _parse_worker_wrapper(args_tuple):
    fp, non_gamma_div, want_proc = args_tuple
    return _parse_one_sim_for_rpip(fp, non_gamma_div, want_proc)


def parse_rpip_points_mp(sim_files: list[str], non_gamma_div: float, want_proc: str,
                         workers: int, chunksize: int):
    """Return merged_points[(VN,ZA,exc)] = list of (x_mm,y_mm,z_mm,w)"""
    merged = defaultdict(list)
    total_lines = 0

    task_args = [(f, non_gamma_div, want_proc) for f in sim_files]
...
def main():
    ...
    rpip_merged, n_lines = parse_rpip_points_mp(
        sim_files, args.non_gamma_div, want_proc="RP",
