...
def build_z_audit(bounds: dict) -> list[dict[str, str]]:
    ...
    for can in bounds["OPEN_BOTTOM_CANS"]:
        ...
        note = (
            ...
            if windows
            else "Current DEMO2 Nb can has an open matched top aperture; no continuous Nb window is assigned."
        )
        rows.append(
            row_for(
                can["name"],
    ...
    for shell in bounds["CRYOSTAT_SHELLS"]:
        note = "Thin window center lies inside the shell top-cap z interval."
        if shell["name"] == "Vacuum_Jacket_Al":
            note = (
                "Be window center lies inside the vacuum-jacket top cap; "
                "Win_Vacuum_Al_Filter is an external low-Z filter proxy."
            )
        rows.append(
            row_for(
                shell["name"],
...
def vrml_color(material: str, name: str) -> tuple[float, float, float, float]:
    ...
    if material in {"CeBr3", "CsI"}:
        return 0.40, 0.70, 0.32, 0.75
    if material == "Cryoperm":
        return 0.35, 0.38, 0.70, 0.55
    if material in {"LowCarbonSteel", "StainlessSteel", "W"}:
        return 0.42, 0.42, 0.42, 0.45
    if material == "SaltProxy":
        return 0.35, 0.66, 0.62, 0.45
    if material == "Kapton":
        return 0.74, 0.42, 0.12, 0.45
    if material == "G10":
        return 0.38, 0.55, 0.34, 0.45
    if material == "Be":
...
def proxy_node(rec: dict) -> str:
    name = str(rec["name"])
    material = str(rec.get("mat", ""))
    if all(k in rec for k in ("hx", "hy", "hz", "x", "y", "z")):
        return box_node(
            name,
            float(rec["x"]),
            float(rec["y"]),
    ...
    if "r" in rec and all(k in rec for k in ("z0", "z1")):
        return cylinder_node(name, float(rec["r"]), float(rec["z0"]), float(rec["z1"]), material)
    if "r_out" in rec:
        z0 = float(rec.get("z0", rec.get("z_out_bot", rec.get("z_in_bot"))))
        z1 = float(rec.get("z1", rec.get("z_out_top", rec.get("z_in_top"))))
        return cylinder_node(name, float(rec["r_out"]), z0, z1, material)
    return ""


def unique_proxy_records(bounds: dict) -> list[dict]:
    records = []
    seen = set()
...
def write_wrl(bounds: dict) -> None:
    ...
    for rec in unique_proxy_records(bounds):
        node = proxy_node(rec)
        if node:
            lines.append(node)
