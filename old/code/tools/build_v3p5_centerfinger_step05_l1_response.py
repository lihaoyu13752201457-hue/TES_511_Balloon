...
def is_fix5_label(label: str) -> bool:
    return label == FIX5_LABEL


def is_fix5_fullstat_label(label: str) -> bool:
    return label in ("fix5_fullstat_v2", FIX5_FULLSTAT_LABEL)


def is_any_fix5_label(label: str) -> bool:
    return is_fix5_label(label) or is_fix5_fullstat_label(label)
...
def configure_paths(label: str) -> None:
    ...
    if is_fix5_label(label):
        OUT = ROOT / "stepwise_maintenance" / "step05_veto_time_axis" / "outputs_fix5_1of10_l1"
        SUMMARY_JSON = OUT / "step05_fix5_1of10_l1_response_summary.json"
        SUMMARY_MD = OUT / "step05_fix5_1of10_l1_response_summary.md"
        RATES_CSV = OUT / "step05_fix5_1of10_l1_rates.csv"
        TIMELINE_CSV = OUT / "step05_fix5_1of10_l1_timeline_rates.csv"
        PROMPT_DIR = ROOT / "runs" / "step02_instant_fix5_1of10"
        PROMPT_NORM = PROMPT_DIR / "normalization.json"
        DELAYED_SIM = ROOT / "runs" / "step02_delayed_transport_fix5_1of10" / "DelayedDecayFix5Exactpos.inc1.id1.sim.gz"
        FIXED_SOURCE = ROOT / "runs" / "step02_delay_fix_fix5_1of10" / "activation_decay_day15_groundstate_fixed_exactpos.source"
        STEP02_SUMMARY = ROOT / "outputs" / "reports" / "fix5_1of10" / "fix5_delayed_source_exactpos_summary.json"
        BOUNDARY_CLOSURE_SUMMARY = (
            ROOT
            / "outputs"
            / "reports"
    ...
    if is_fix5_fullstat_label(label):
        OUT = ROOT / "stepwise_maintenance" / "step05_veto_time_axis" / "outputs_fix5_fullstat_v2_exactpos_m50000_s260613_l1"
        SUMMARY_JSON = OUT / "step05_fix5_fullstat_v2_exactpos_m50000_s260613_l1_response_summary.json"
        SUMMARY_MD = OUT / "step05_fix5_fullstat_v2_exactpos_m50000_s260613_l1_response_summary.md"
        RATES_CSV = OUT / "step05_fix5_fullstat_v2_exactpos_m50000_s260613_l1_rates.csv"
        TIMELINE_CSV = OUT / "step05_fix5_fullstat_v2_exactpos_m50000_s260613_l1_timeline_rates.csv"
        PROMPT_DIR = ROOT / "runs" / "step02_instant_fix5_fullstat_v2"
        PROMPT_NORM = PROMPT_DIR / "normalization.json"
        DELAYED_SIM = (
            ROOT
            / "runs"
        ...
        FIXED_SOURCE = ROOT / "runs" / "step02_delay_fix_fix5_fullstat_v2" / "activation_decay_day15_groundstate_fixed_exactpos_m50000_s260613.source"
        STEP02_SUMMARY = ROOT / "outputs" / "reports" / "fix5_fullstat_v2_exactpos_m50000_s260613" / "fix5_delayed_source_exactpos_summary.json"
        SCIENCE_SIM = (
            ROOT
            / "runs"
            / "step09_focus_fix5_fullstat_v2_exactpos_m50000_s260613"
            / "Opticsim_laue_f10m_a1_fix5_fullstat_v2_exactpos_m50000_s260613.inc1.id1.sim.gz"
        )
        STEP09_SUMMARY = (
            ROOT
...
def add_physical_reference_to_windows(windows: dict[str, Any], norm: dict[str, Any]) -> None:
    ...
    for item in windows.values():
        ...
        item["physical_reference_flux"] = {
            ...
            "claim_note": "Direct expectation only. It uses the current Step05 selected-event rates and does not replace a full Step06-Step08 mission-axis regeneration.",
        }
...
def markdown(payload: dict[str, Any]) -> str:
    def fmt_optional(value: Any, precision: str = ".6g") -> str:
        if value is None:
            return "NA"
        try:
    ...
    if is_bgo_sample_label(label):
        title = "# Step05 Bgo_sample L1 Detector Response"
        claim = (
        ...
        title = "# Step05 fix5 Full-Stat L1 Detector Response"
        claim = "fix5 full-stat prompt, exact-position delayed, and fix5 focused-signal detector-response extraction; not a promotion decision until Step06--Step08 and the promotion artifact close."
    elif is_fix5_label(label):
        title = "# Step05 fix5 1/10 L1 Detector Response"
        claim = "fix5 1/10 prompt/delayed detector-response screen; not a promotion authority and not an old new_geo_re gate."
    else:
        title = "# Step05 v3p5 Center-Finger L1 Detector Response"
        claim = "v3p5 side-entry Compton/FoV migrated to the tilted Be disk, plus direct expectation, physical reference-flux scaling, and one common Poisson time-axis draw."
    lines = [
        title,
        "",
    ...
    for win, item in payload["windows"].items():
        ...
        lines.extend(
            [
                ...
                f"- Z20d direct S/sqrt(B): `{fmt_optional(phys['Z20d_direct_s_over_sqrt_b'])}`; T3 constant-rate direct: `{fmt_optional(phys['T3_day_constant_rate_direct'])} day`; 20-day 3-sigma flux: `{fmt_optional(phys['flux_3sigma_20d_ph_cm2_s'])} ph cm^-2 s^-1`",
                f"- low-stat final background events: `{phys['low_stat_final_background_events']}`; approximate relative Poisson sigma `{fmt_optional(phys['low_stat_background_relative_poisson_sigma_approx'])}`",
                "",
            ]
        )
    lines.extend(
    ...
    pending_title = "Pending before fix5 promotion:" if is_any_fix5_label(label) else "Pending before paper-facing v3p5 statistics:"
    lines.extend(["", pending_title])
...
def main() -> int:
    ...
    elif is_fix5_fullstat_label(args.label):
        pending = [
    ...
    if is_bgo_sample_label(args.label):
        status = "PASS_BGO_SAMPLE_STEP05_SIDE_ENTRY_COMPTON_TIME_AXIS_L1_FULLSTAT_V2_EXACTPOS"
        claim_level = "BGO_SAMPLE_L1_DETECTOR_RESPONSE_FULLSTAT_V2_EXACTPOS_NOT_STEP08_SENSITIVITY"
    elif is_fix5_fullstat_label(args.label):
        status = "PASS_FIX5_STEP05_SIDE_ENTRY_COMPTON_TIME_AXIS_L1_FULLSTAT_V2_EXACTPOS_NOT_PROMOTION"
        claim_level = "FIX5_FULLSTAT_V2_EXACTPOS_L1_PROMPT_DELAYED_NOT_FINAL_PROMOTION"
    elif is_fix5_label(args.label):
        status = "PASS_FIX5_STEP05_SIDE_ENTRY_COMPTON_TIME_AXIS_L1_1OF10_NOT_PROMOTION"
        claim_level = "FIX5_1OF10_L1_PROMPT_DELAYED_SCREEN_NOT_FINAL_GATE"
    else:
