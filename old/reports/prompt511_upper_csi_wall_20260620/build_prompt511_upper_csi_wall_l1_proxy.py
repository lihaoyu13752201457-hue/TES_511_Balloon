#!/usr/bin/env python3
"""Build a prompt Step05-like L1 proxy for the upper-CsI-wall smoke."""
...
UPPER_WALL_PROMPT_DIR_BY_TAG = {
    "eplus": WORK / "runs/upper_csi_wall_eplus_g10m_r4_cli_seed",
    "n": WORK / "runs/upper_csi_wall_n_g10m_r16_cli_seed",
    "muplus": WORK / "runs/upper_csi_wall_muplus_g10m_r80_cli_seed",
}

SUMMARY_JSON = WORK / "prompt511_upper_csi_wall_l1_proxy_summary.json"
SUMMARY_MD = WORK / "prompt511_upper_csi_wall_l1_proxy_summary.md"
...
def summarize_particle_cases(proxy, step05, disk: dict[str, Any], reject_policy: str) -> dict[str, Any]:
    ...
    for tag, prompt_dir in UPPER_WALL_PROMPT_DIR_BY_TAG.items():
        ...
        particle_cases[tag] = {
            ...
            "upper_csi_wall": proxy.summarize_prompt_dir(step05, prompt_dir, disk, reject_policy, tag=tag),
        }
    if "eplus" not in particle_cases:
        raise RuntimeError("eplus upper-CsI-wall case is required")
...
def build_projection(proxy, step05, disk: dict[str, Any], reject_policy: str, particle_cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for tag, pair in particle_cases.items():
        projected_rates[tag] = float(pair["upper_csi_wall"]["summary"]["side_compton_fov_pass_rate_s-1"])
    ...
    return {
        ...
        "upper_csi_wall_replaced_tags": sorted(particle_cases),
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    for tag, pair in payload["particle_cases"].items():
        for case_name in ("current_baseline", "upper_csi_wall"):
    ...
    lines.extend(["", "Current/upper-CsI-wall suppression factors:", ""])
    for tag, stages in payload["particle_ratios"].items():
        for stage, row in stages.items():
            r = row["current_over_upper_csi_wall_ratio"]
            if r is None:
                continue
    ...
    lines.extend(
        [
            ...
            f"- projected prompt total with upper-CsI-wall replaced tags: `{proj['projected_total_s-1']:.6g} cps`.",
            ...
            f"- upper-CsI-wall replaced tags: `{tag_list(proj['upper_csi_wall_replaced_tags'])}`.",
...
def main() -> int:
    proxy = load_module("prompt511_repack_l1_proxy_base_for_upper_csi_wall", BASE_PROXY)
    step05 = load_module(
        "build_v3p5_centerfinger_step05_l1_response_for_upper_csi_wall",
    ...
    status = (
        "PASS_PROMPT511_UPPER_CSI_WALL_L1_PROXY"
        if {"eplus", "n", "muplus"}.issubset(particle_cases)
        else "PASS_PROMPT511_UPPER_CSI_WALL_L1_PROXY_PARTIAL"
    )
    payload = {
        "status": status,
        "claim_level": "prompt-only upper-CsI-wall design smoke; not rate authority",
