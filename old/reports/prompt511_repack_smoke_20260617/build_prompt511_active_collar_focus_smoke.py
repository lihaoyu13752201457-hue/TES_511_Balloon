#!/usr/bin/env python3
"""Build and summarize active-collar focused EventList transport smoke."""
...
GEOMETRY = (
    WORK
    / "geometry_active_collar_bgo"
    / "DEMO2_DR_v3p5_minpatch_centerfinger_megalib_proxy_prompt511_active_bgo_collar_r4p25_5p95.geo.setup"
)
...
RUN_DIR = WORK / "runs/active_collar_bgo_focus_smoke"
RUN_NAME = "Opticsim_laue_f10m_a1_prompt511_active_bgo_collar"
SOURCE = RUN_DIR / f"{RUN_NAME}.source"
PREFIX = RUN_DIR / RUN_NAME
SIM = PREFIX.with_suffix(".inc1.id1.sim.gz")
SUMMARY_JSON = WORK / "prompt511_active_collar_bgo_focus_smoke_summary.json"
SUMMARY_MD = WORK / "prompt511_active_collar_bgo_focus_smoke_summary.md"
...
def source_text(triggers: int, seed: int) -> str:
    return f"""# Auto-generated prompt-511 active-collar focused EventList smoke.
# Replays the current f10m A1 v3p5 Step09 EventList through active BGO collar geometry.
...
def build_source(seed: int) -> dict[str, Any]:
    ...
    payload = {
        "status": "PASS_PROMPT511_ACTIVE_COLLAR_FOCUS_SOURCE_READY",
        "claim_level": "focused EventList source for active-collar signal smoke; not response authority",
        "source": rel(SOURCE),
        "geometry": rel(GEOMETRY),
        ...
        "boundary": [
            "This source replays the current f10m A1 v3p5 focused EventList through the active BGO collar geometry.",
            "It is a signal-clipping smoke only until detector response and mission-time chains consume the result.",
        ],
    }
...
def summarize_transport_and_signal() -> dict[str, Any]:
    ...
    if not transport["exists"]:
        problems.append("missing_active_collar_focus_transport_sim")
    if int(transport["SE"]) != int(transport["ID"]):
        problems.append("active_collar_SE_ID_mismatch")
    if int(transport["SE"]) != triggers:
        problems.append("active_collar_stored_events_do_not_match_eventlist_triggers")
    if rel(GEOMETRY) not in str(transport["geometry"]) and str(GEOMETRY) not in str(transport["geometry"]):
        problems.append("active_collar_transport_geometry_mismatch")

    payload["focused_transport"] = transport
    payload["current_focused_transport"] = current_transport
    if transport["exists"]:
        proxy = load_module("prompt511_focus_smoke_base_proxy", BASE_PROXY)
        step05 = load_module(
        ...
        payload["focused_signal_proxy"] = {
            ...
            "active_collar_bgo": active_summary,
    ...
    if problems:
        payload["status"] = "FAIL_PROMPT511_ACTIVE_COLLAR_FOCUS_SMOKE"
    else:
        payload["status"] = "PASS_PROMPT511_ACTIVE_COLLAR_FOCUS_SMOKE"
    ...
    if proxy:
        ...
        lines.extend(
            [
                ...
                f"| active_collar_bgo | {var['generated_events_seen']} | {var['tes_events_kept']} | {var['raw_events']} | {var['active_veto_pass_events']} | {var['side_compton_fov_pass_events']} |",
                "",
...
def main() -> int:
    ...
    build.add_argument("--seed", type=int, default=260619)
