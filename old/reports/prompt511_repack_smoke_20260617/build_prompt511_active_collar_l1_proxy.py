#!/usr/bin/env python3
"""Build a prompt Step05-like L1 proxy for the active BGO collar smoke."""
...
ACTIVE_PROMPT_DIR_BY_TAG = {
    "eplus": WORK / "runs/active_collar_bgo_eplus_g10m_r4_cli_seed",
    "n": WORK / "runs/active_collar_bgo_n_g10m_r16_cli_seed",
    "muplus": WORK / "runs/active_collar_bgo_muplus_g10m_r80_cli_seed",
}

SUMMARY_JSON = WORK / "prompt511_active_collar_bgo_l1_proxy_summary.json"
SUMMARY_MD = WORK / "prompt511_active_collar_bgo_l1_proxy_summary.md"
...
def run_ready(prompt_dir: Path, tag: str, proxy) -> bool:
    return proxy.repack_dir_ready(prompt_dir, tag)


def summarize_particle_cases(proxy, step05, disk: dict[str, Any], reject_policy: str) -> dict[str, Any]:
    particle_cases: dict[str, Any] = {}
    for tag, prompt_dir in ACTIVE_PROMPT_DIR_BY_TAG.items():
        if not run_ready(prompt_dir, tag, proxy):
            continue
        particle_cases[tag] = {
            "current_baseline": proxy.summarize_catalog_case(
                step05, CURRENT_STEP05_CACHE, disk, reject_policy, stream="prompt", tag=tag
            ),
            "active_collar_bgo": proxy.summarize_prompt_dir(
                step05, prompt_dir, disk, reject_policy, tag=tag
            ),
        }
    if "eplus" not in particle_cases:
        raise RuntimeError("eplus active-collar case is required")
...
def build_projection(proxy, step05, disk: dict[str, Any], reject_policy: str, particle_cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for tag, pair in particle_cases.items():
        projected_rates[tag] = float(pair["active_collar_bgo"]["summary"]["side_compton_fov_pass_rate_s-1"])
    ...
    return {
        ...
        "active_collar_replaced_tags": sorted(particle_cases),
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    for tag, pair in payload["particle_cases"].items():
        for case_name in ("current_baseline", "active_collar_bgo"):
    ...
    lines.extend(["", "Current/active-collar suppression factors:", ""])
    for tag, stages in payload["particle_ratios"].items():
        for stage, row in stages.items():
            r = row["current_over_active_collar_ratio"]
            if r is None:
                continue
    ...
    lines.extend(
        [
            ...
            f"- projected prompt total with active-collar replaced tags: `{proj['projected_total_s-1']:.6g} cps`.",
            ...
            f"- active-collar replaced tags: `{tag_list(proj['active_collar_replaced_tags'])}`.",
...
def main() -> int:
    proxy = load_module("prompt511_repack_l1_proxy_base", BASE_PROXY)
    step05 = load_module(
        "build_v3p5_centerfinger_step05_l1_response_for_active_collar",
    ...
    status = (
        "PASS_PROMPT511_ACTIVE_COLLAR_BGO_L1_PROXY"
        if "n" in particle_cases
        else "PASS_PROMPT511_ACTIVE_COLLAR_BGO_L1_PROXY_EPLUS_ONLY"
    )
    payload = {
        "status": status,
        "claim_level": "prompt-only active-collar design smoke; not rate authority",
