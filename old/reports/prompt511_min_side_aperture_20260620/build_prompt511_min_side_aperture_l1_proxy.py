#!/usr/bin/env python3
"""Build a prompt Step05-like L1 proxy for the minimum side-aperture candidate.

This is a prompt-only diagnostic.  It reuses the official v3p5 TES line
window, CsI active-veto matching, and side-entry Compton/FoV proxy, but it does
not mix in delayed activation, focused signal, or a mission time axis.
"""
...
MIN_APERTURE_PROMPT_DIR_BY_TAG = {
    "eplus": WORK / "runs/min_side_aperture_eplus_g10m_r4_promptonly",
    "n": WORK / "runs/min_side_aperture_n_g10m_r16_promptonly",
    "muplus": WORK / "runs/min_side_aperture_muplus_g10m_r80_promptonly",
}

SUMMARY_JSON = WORK / "prompt511_min_side_aperture_l1_proxy_summary.json"
SUMMARY_MD = WORK / "prompt511_min_side_aperture_l1_proxy_summary.md"
...
def summarize_particle_cases(proxy, step05, disk: dict[str, Any], reject_policy: str) -> dict[str, Any]:
    ...
    for tag, prompt_dir in MIN_APERTURE_PROMPT_DIR_BY_TAG.items():
        ...
        particle_cases[tag] = {
            ...
            "min_side_aperture": proxy.summarize_prompt_dir(step05, prompt_dir, disk, reject_policy, tag=tag),
        }
    if "eplus" not in particle_cases:
        raise RuntimeError("eplus min-side-aperture case is required")
...
def build_projection(proxy, step05, disk: dict[str, Any], reject_policy: str, particle_cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for tag, pair in particle_cases.items():
        projected_rates[tag] = float(pair["min_side_aperture"]["summary"]["side_compton_fov_pass_rate_s-1"])
    ...
    return {
        ...
        "min_side_aperture_replaced_tags": sorted(particle_cases),
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Prompt-511 Minimum Side-Aperture L1-like Proxy",
        "",
        f"Status: `{payload['status']}`",
        "",
        "This is a prompt-only Step05-like diagnostic for the minimum",
        "side-aperture geometry candidate. It reuses the official v3p5 line",
    ...
    for tag, pair in payload["particle_cases"].items():
        for case_name in ("current_baseline", "min_side_aperture"):
    ...
    lines.extend(["", "Current/min-side-aperture suppression factors:", ""])
    for tag, stages in payload["particle_ratios"].items():
        for stage, row in stages.items():
            r = row["current_over_min_side_aperture_ratio"]
            if r is None:
                continue
    ...
    lines.extend(
        [
            ...
            f"- projected prompt total with min-side-aperture replaced tags: `{proj['projected_total_s-1']:.6g} cps`.",
            ...
            f"- min-side-aperture replaced tags: `{tag_list(proj['min_side_aperture_replaced_tags'])}`.",
...
def main() -> int:
    proxy = load_module("prompt511_repack_l1_proxy_base_for_min_side_aperture", BASE_PROXY)
    install_prompt_only_rate_fallback(proxy)
    step05 = load_module(
        "build_v3p5_centerfinger_step05_l1_response_for_min_side_aperture",
    ...
    status = (
        "PASS_PROMPT511_MIN_SIDE_APERTURE_L1_PROXY"
        if {"eplus", "n", "muplus"}.issubset(particle_cases)
        else "PASS_PROMPT511_MIN_SIDE_APERTURE_L1_PROXY_PARTIAL"
    )
    payload = {
        "status": status,
        "claim_level": "prompt-only min-side-aperture design smoke; not rate authority",
        ...
        "limitations": [
            "Only completed min-side-aperture prompt particle runs are parsed.",
