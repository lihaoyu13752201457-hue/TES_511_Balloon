#!/usr/bin/env python3
"""Build a prompt Step05-like L1 proxy for the inner active-CsI shroud smoke."""
...
INNER_CSI_SHROUD_PROMPT_DIR_BY_TAG = {
    "eplus": WORK / "runs/inner_csi_shroud_eplus_g10m_r4_promptonly",
    "n": WORK / "runs/inner_csi_shroud_n_g10m_r16_promptonly",
    "muplus": WORK / "runs/inner_csi_shroud_muplus_g10m_r80_promptonly",
}

SUMMARY_JSON = WORK / "prompt511_inner_csi_shroud_l1_proxy_summary.json"
SUMMARY_MD = WORK / "prompt511_inner_csi_shroud_l1_proxy_summary.md"
...
def prompt_only_rates_from_norm(prompt_dir: Path) -> dict[str, float]:
    ...
    return {
        tag: flux_by_tag[tag] * area / events
...
def summarize_particle_cases(proxy, step05, disk: dict[str, Any], reject_policy: str) -> dict[str, Any]:
    ...
    for tag, prompt_dir in INNER_CSI_SHROUD_PROMPT_DIR_BY_TAG.items():
        ...
        particle_cases[tag] = {
            ...
            "inner_csi_shroud": proxy.summarize_prompt_dir(step05, prompt_dir, disk, reject_policy, tag=tag),
        }
    if "eplus" not in particle_cases:
        raise RuntimeError("eplus inner-CsI-shroud case is required")
...
def build_projection(proxy, step05, disk: dict[str, Any], reject_policy: str, particle_cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for tag, pair in particle_cases.items():
        projected_rates[tag] = float(pair["inner_csi_shroud"]["summary"]["side_compton_fov_pass_rate_s-1"])
    ...
    return {
        ...
        "inner_csi_shroud_replaced_tags": sorted(particle_cases),
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    for tag, pair in payload["particle_cases"].items():
        for case_name in ("current_baseline", "inner_csi_shroud"):
    ...
    lines.extend(["", "Current/inner-CsI-shroud suppression factors:", ""])
    for tag, stages in payload["particle_ratios"].items():
        for stage, row in stages.items():
            r = row["current_over_inner_csi_shroud_ratio"]
            if r is None:
                continue
    ...
    lines.extend(
        [
            ...
            f"- projected prompt total with inner-CsI-shroud replaced tags: `{proj['projected_total_s-1']:.6g} cps`.",
            ...
            f"- inner-CsI-shroud replaced tags: `{tag_list(proj['inner_csi_shroud_replaced_tags'])}`.",
...
def main() -> int:
    proxy = load_module("prompt511_repack_l1_proxy_base_for_inner_csi_shroud", BASE_PROXY)
    install_prompt_only_rate_fallback(proxy)
    step05 = load_module(
        "build_v3p5_centerfinger_step05_l1_response_for_inner_csi_shroud",
    ...
    status = (
        "PASS_PROMPT511_INNER_CSI_SHROUD_L1_PROXY"
        if {"eplus", "n", "muplus"}.issubset(particle_cases)
        else "PASS_PROMPT511_INNER_CSI_SHROUD_L1_PROXY_PARTIAL"
    )
    payload = {
        "status": status,
        "claim_level": "prompt-only inner-CsI-shroud design smoke; not rate authority",
    ...
    print(json.dumps({"status": status, "summary_json": rel(SUMMARY_JSON), "summary_md": rel(SUMMARY_MD)}, indent=2))
    return 0
