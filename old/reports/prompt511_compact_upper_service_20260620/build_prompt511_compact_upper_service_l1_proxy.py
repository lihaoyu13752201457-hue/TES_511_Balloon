#!/usr/bin/env python3
"""Build a prompt Step05-like L1 proxy for the compact upper-service candidate.
...
COMPACT_PROMPT_DIR_BY_TAG = {
    "eplus": WORK / "runs/compact_upper_service_eplus_g10m_r4_promptonly",
    "n": WORK / "runs/compact_upper_service_n_g10m_r16_promptonly",
    "muplus": WORK / "runs/compact_upper_service_muplus_g10m_r80_promptonly",
}

SUMMARY_JSON = WORK / "prompt511_compact_upper_service_l1_proxy_summary.json"
SUMMARY_MD = WORK / "prompt511_compact_upper_service_l1_proxy_summary.md"
...
def summarize_particle_cases(proxy, step05, disk: dict[str, Any], reject_policy: str) -> dict[str, Any]:
    ...
    for tag, prompt_dir in COMPACT_PROMPT_DIR_BY_TAG.items():
        ...
        particle_cases[tag] = {
            ...
            "compact_upper_service": proxy.summarize_prompt_dir(step05, prompt_dir, disk, reject_policy, tag=tag),
        }
    if "eplus" not in particle_cases:
        raise RuntimeError("eplus compact-upper-service case is required")
...
def build_projection(proxy, step05, disk: dict[str, Any], reject_policy: str, particle_cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for tag, pair in particle_cases.items():
        projected_rates[tag] = float(pair["compact_upper_service"]["summary"]["side_compton_fov_pass_rate_s-1"])
    ...
    return {
        ...
        "compact_upper_service_replaced_tags": sorted(particle_cases),
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    for tag, pair in payload["particle_cases"].items():
        for case_name in ("current_baseline", "compact_upper_service"):
    ...
    lines.extend(["", "Current/compact-upper-service suppression factors:", ""])
    for tag, stages in payload["particle_ratios"].items():
        for stage, row in stages.items():
            r = row["current_over_compact_upper_service_ratio"]
            if r is None:
                continue
    ...
    lines.extend(
        [
            ...
            f"- projected prompt total with compact-upper-service replaced tags: `{proj['projected_total_s-1']:.6g} cps`.",
            ...
            f"- compact-upper-service replaced tags: `{tag_list(proj['compact_upper_service_replaced_tags'])}`.",
...
def main() -> int:
    proxy = load_module("prompt511_repack_l1_proxy_base_for_compact_upper_service", BASE_PROXY)
    install_prompt_only_rate_fallback(proxy)
    step05 = load_module(
        "build_v3p5_centerfinger_step05_l1_response_for_compact_upper_service",
    ...
    status = (
        "PASS_PROMPT511_COMPACT_UPPER_SERVICE_L1_PROXY"
        if {"eplus", "n", "muplus"}.issubset(particle_cases)
        else "PASS_PROMPT511_COMPACT_UPPER_SERVICE_L1_PROXY_PARTIAL"
    )
    payload = {
        "status": status,
        "claim_level": "prompt-only compact-upper-service design smoke; not rate authority",
