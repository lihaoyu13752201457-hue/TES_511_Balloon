...
- Legacy CsI detector-coupled `spot_r90` sidecar (Step06 time fold): background
  `0.0551005 cps`, `Z20d=4.50779`, `T3=8.179 day`, 20-day 3-sigma flux
  `6.655e-5 ph cm-2 s-1` (constant-rate variant: `Z20d=4.52748`,
  flux `6.626e-5`).

2026-06-18 correction: do not quote `spot_r90` as a prompt-suppression
strategy or paper headline. It is retained only as a spatial diagnostic; the
current prompt-fix direction is local side-port/side-wall geometry closure, and
the current primary rate authority remains the hard-window
`fullstat_v2_exactpos_m50000_s260613` chain.
...
- Current BGO equal-attenuation material-comparison branch is local
  ...
  `F3(20d)=4.66219e-5 ph cm-2 s-1`. The material-control comparison report
...
defensible headline claim should use the current hard-window exact-position
authority, not a `spot_r90` analysis sidecar. The older `~6.7e-5 ph cm-2 s-1`
value was a spatial-cut sidecar and must not be promoted as the paper headline.
...
1. `laue.png` lens-reference cleanup.
   ...
   - Ge(111), `f=10 m`, Bragg-annulus estimate: `A_eff ~18 cm2`.
     Do not use the old `spot_r90` 1 Ms sidecar estimates as prompt-fix
     strategy numbers.
   - If a 511-tuned lens is designed to `A_eff=20 cm2`, it is only a modest
     upgrade. A current-efficiency
   ...
   - If `100 cm2` is only imaging/focal-plane area, it does not increase signal
     ...
     current-efficiency scaling would give `A_eff ~25 cm2`; any 1 Ms flux
     estimate should be recomputed against the current hard-window authority.
