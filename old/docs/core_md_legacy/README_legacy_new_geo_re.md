...
After the prompt-entry audit, do not present this ROI sidecar as the
prompt-suppression strategy; the physical fix direction is local
side-port/side-wall geometry closure.
...
- legacy diagnostic cut is `spot_r90`, not `spot_rmax`: signal fraction
  `0.9000`, background `0.05510 cps`, `Z20d = 4.527`;
- 20-day 3-sigma flux with spot_r90 was `6.63e-5 ph cm^-2 s^-1` in that
  diagnostic sidecar; do not promote it as the current prompt-suppression
  strategy or paper headline.
