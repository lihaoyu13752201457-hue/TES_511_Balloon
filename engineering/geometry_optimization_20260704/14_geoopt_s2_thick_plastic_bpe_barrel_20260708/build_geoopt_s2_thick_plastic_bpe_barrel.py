#!/usr/bin/env python3
"""Build the S2 thick plastic/BPE barrel draft from Mass_model_511.
...
WRL = FIG_DIR / "geoopt_s2_thick_plastic_bpe_barrel.wrl"
DETAIL_PNG = FIG_DIR / "geoopt_s2_thick_plastic_bpe_barrel_2d_detail.png"
DETAIL_SVG = FIG_DIR / "geoopt_s2_thick_plastic_bpe_barrel_2d_detail.svg"
README = WORK / "README.md"
MANIFEST = WORK / "geoopt_s2_thick_plastic_bpe_barrel_manifest.json"
...
PATCH_STATUS = "DRAFT_S2_THICK_PLASTIC_BPE_BARREL_NOT_TRANSPORT_VALIDATED"

# User-requested S2 patch dimensions.  The layer order is:
# outside -> inside: 10 mm plastic scintillator skin, 20 mm borated PE,
# retained 5 mm W bottom baffle, then the unmodified Mass_model_511 detector.
PLASTIC_THICKNESS_CM = 1.00
PLASTIC_RIN_CM = 28.40
PLASTIC_ROUT_CM = 29.40
PLASTIC_CAP_ROUT_CM = PLASTIC_RIN_CM
PLASTIC_SIDE_ZMIN_CM = -25.10
PLASTIC_SIDE_ZMAX_CM = 9.20
PLASTIC_BOTTOM_ZMIN_CM = -25.10
PLASTIC_BOTTOM_ZMAX_CM = -24.10
PLASTIC_TOP_ZMIN_CM = 8.20
PLASTIC_TOP_ZMAX_CM = 9.20

BPE_THICKNESS_CM = 2.00
BPE_RIN_CM = 26.40
BPE_ROUT_CM = 28.40
BPE_CAP_ROUT_CM = BPE_RIN_CM
BPE_BOTTOM_CAP_RIN_CM = 0.00
BPE_SIDE_ZMIN_CM = -24.10
BPE_SIDE_ZMAX_CM = 8.20
BPE_BOTTOM_ZMIN_CM = -24.10
BPE_BOTTOM_ZMAX_CM = -22.10
BPE_TOP_ZMIN_CM = 6.20
BPE_TOP_ZMAX_CM = 8.20
BPE_DENSITY_G_CM3 = 0.95

TOP_CAP_RIN_CM = 0.00
TOP_ANNULAR_RELIEFS = (
    ("DRStillPot", 0.00, 3.05),
    ("StillShieldSideWall", 15.30, 16.00),
    ("Shield4KSideWall", 17.50, 18.10),
    ("Shield60KSideWall", 18.00, 18.70),
    ("VacuumJacketSideWall", 19.80, 20.90),
)
TOP_SUPPORT_ROD_RELIEF_HALF_X_CM = 1.30
TOP_SUPPORT_ROD_RELIEF_HALF_Y_CM = 1.30
TOP_SUPPORT_ROD_RELIEF_EXTRA_HALF_Z_CM = 0.20
TOP_SUPPORT_ROD_RELIEF_CENTERS_CM = (
    (6.19112313, 2.25338454),
    (-6.19112313, -2.25338454),
    (-1.14407331, -6.48836217),
    (5.04704982, -4.23497764),
)
TOP_SECTOR_RELIEFS = (
    ("DRHexCPStill", 0.0, 108.0, 5.00, 5.70),
    ("DRCapillaryCPStill", 38.4, 43.2, 6.00, 6.50),
    ("NbTiBundleCPStill", 99.4, 61.2, 6.40, 7.10),
    ("G10SupportRingCPStill", 210.0, 90.0, 7.80, 8.60),
)

BOTTOM_ROD_RELIEF_X_CM = -19.50
BOTTOM_ROD_RELIEF_Y_CM = 19.80
BOTTOM_ROD_RELIEF_HALF_X_CM = 6.00
BOTTOM_ROD_RELIEF_HALF_Y_CM = 6.00
BOTTOM_ROD_RELIEF_HALF_Z_CM = 0.70
SIDE_ROD_RELIEF_Z_LOCAL_CM = -14.90
SIDE_ROD_RELIEF_HALF_X_CM = 6.00
SIDE_ROD_RELIEF_HALF_Y_CM = 6.00
SIDE_ROD_RELIEF_HALF_Z_CM = 3.00
...
def bpe_side_window_shape_defs(name: str) -> tuple[str, ...]:
    ...
    window_sub = f"{name}_WindowSubtractionShape"
    ...
    return (
        ...
        f"Shape Subtraction {window_sub}",
        f"{window_sub}.Parameters {full} {cut} {orient}",
        f"Shape BRIK {cut_a}",
        ...
        f"{sub_a}.Parameters {window_sub} {cut_a} {orient_a}",
...
def plastic_bottom_rod_relief_shape_defs(name: str) -> tuple[str, ...]:
    planes, _ = global_pcon_planes(PLASTIC_BOTTOM_ZMIN_CM, PLASTIC_BOTTOM_ZMAX_CM, 0.0, PLASTIC_CAP_ROUT_CM)
...
def top_cap_service_relief_shape_defs(
    name: str, z_min: float, z_max: float, r_outer: float
) -> tuple[tuple[str, ...], str]:
    planes, _ = global_pcon_planes(z_min, z_max, TOP_CAP_RIN_CM, r_outer)
    z_half = 0.5 * abs(z_max - z_min)
    cut_half_z = z_half + TOP_SUPPORT_ROD_RELIEF_EXTRA_HALF_Z_CM
    full = f"{name}_FullDiskShape"
    lines = [
        f"Shape PCON {full}",
        f"{full}.Parameters {' '.join(pcon_line(0.0, 360.0, planes).split()[1:])}",
    ...
    previous = full
    for idx, (label, rin, rout) in enumerate(TOP_ANNULAR_RELIEFS, start=1):
        cut = f"{name}_{label}AnnularReliefShape"
        orient = f"{name}_{label}AnnularReliefOrientation"
        sub = f"{name}_AnnularReliefStep{idx:02d}Shape"
        relief_planes = [(-cut_half_z, rin, rout), (cut_half_z, rin, rout)]
        lines.extend(
            [
                f"Shape PCON {cut}",
                f"{cut}.Parameters {' '.join(pcon_line(0.0, 360.0, relief_planes).split()[1:])}",
                f"Orientation {orient}",
                f"{orient}.Position 0 0 0",
                f"Shape Subtraction {sub}",
                f"{sub}.Parameters {previous} {cut} {orient}",
            ]
        )
        previous = sub
    for idx, (label, phi0, dphi, rin, rout) in enumerate(TOP_SECTOR_RELIEFS, start=1):
        cut = f"{name}_{label}ReliefShape"
        orient = f"{name}_{label}ReliefOrientation"
        sub = f"{name}_SectorReliefStep{idx:02d}Shape"
        sector_planes = [(-cut_half_z, rin, rout), (cut_half_z, rin, rout)]
        lines.extend(
            [
                f"Shape PCON {cut}",
                f"{cut}.Parameters {' '.join(pcon_line(phi0, dphi, sector_planes).split()[1:])}",
                f"Orientation {orient}",
                f"{orient}.Position 0 0 0",
                f"Shape Subtraction {sub}",
                f"{sub}.Parameters {previous} {cut} {orient}",
        ...
        previous = sub
    for idx, (x_cm, y_cm) in enumerate(TOP_SUPPORT_ROD_RELIEF_CENTERS_CM, start=1):
        cut = f"{name}_XS400Rod{idx:02d}ReliefShape"
        orient = f"{name}_XS400Rod{idx:02d}ReliefOrientation"
        sub = f"{name}_ReliefStep{idx:02d}Shape"
        lines.extend(
            [
                f"Shape BRIK {cut}",
                (
                    f"{cut}.Parameters {fmt(TOP_SUPPORT_ROD_RELIEF_HALF_X_CM)} "
                    f"{fmt(TOP_SUPPORT_ROD_RELIEF_HALF_Y_CM)} {fmt(cut_half_z)}"
                ),
                f"Orientation {orient}",
                f"{orient}.Position {fmt(x_cm)} {fmt(y_cm)} 0",
                f"Shape Subtraction {sub}",
                f"{sub}.Parameters {previous} {cut} {orient}",
            ]
        )
...
def load_r3_geometry() -> dict[str, float]:
    if not R3_METRICS.exists():
        return {
            "available": False,
            "source": rel(R3_METRICS),
            "note": "R3 provenance metrics were not present in this retained workspace; S2 does not instantiate R3 sectors.",
        }
...
# BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_MATERIALS
...
# END GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_MATERIALS
"""
    if "BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_MATERIALS" not in text:
...
def build_patch_volumes(skin: dict[str, float]) -> list[PatchVolume]:
    ...
    plastic_bottom_planes, plastic_bottom_z = global_pcon_planes(
        PLASTIC_BOTTOM_ZMIN_CM, PLASTIC_BOTTOM_ZMAX_CM, 0.0, PLASTIC_CAP_ROUT_CM
    )
    plastic_top_planes, plastic_top_z = global_pcon_planes(
        PLASTIC_TOP_ZMIN_CM, PLASTIC_TOP_ZMAX_CM, TOP_CAP_RIN_CM, PLASTIC_CAP_ROUT_CM
    )
    ...
    bpe_bottom_planes, bpe_bottom_z = global_pcon_planes(
        BPE_BOTTOM_ZMIN_CM, BPE_BOTTOM_ZMAX_CM, BPE_BOTTOM_CAP_RIN_CM, BPE_CAP_ROUT_CM
    )
    bpe_top_planes, bpe_top_z = global_pcon_planes(
        BPE_TOP_ZMIN_CM, BPE_TOP_ZMAX_CM, TOP_CAP_RIN_CM, BPE_CAP_ROUT_CM
    )
    w_planes, w_z = global_pcon_planes(W_BAFFLE_ZMIN_CM, W_BAFFLE_ZMAX_CM, 0.0, W_BAFFLE_ROUT_CM)
    plastic_top_shape_defs, plastic_top_shape = top_cap_service_relief_shape_defs(
        "GeoOpt_S2_PlasticFullWrap_TopCap_10mm",
        PLASTIC_TOP_ZMIN_CM,
        PLASTIC_TOP_ZMAX_CM,
        PLASTIC_CAP_ROUT_CM,
    )
    bpe_top_shape_defs, bpe_top_shape = top_cap_service_relief_shape_defs(
        "GeoOpt_S2_BPE5_FullWrap_TopCap_20mm",
        BPE_TOP_ZMIN_CM,
        BPE_TOP_ZMAX_CM,
        BPE_CAP_ROUT_CM,
    )
    ...
    plastic_roles = {
        "side": (
            "10 mm full-azimuth active plastic scintillator outer side skin; covers the side-window azimuth as a "
            "charged-particle sentinel instead of using the earlier low-statistics segmented S1 sector proxy"
        ),
        "bottom": "10 mm active plastic scintillator bottom disk closing the outer charged-particle veto envelope",
        "top": "10 mm active plastic scintillator top disk over the former central service/support opening, with local service reliefs",
    }
    for name, planes, z, role, shape, shape_defs in [
        (
            "GeoOpt_S2_PlasticFullWrap_SideShell_10mm",
            plastic_side_planes,
            plastic_side_z,
            plastic_roles["side"] + "; includes two local NF2 outer-support rod relief cutouts near the lower edge",
            "GeoOpt_S2_PlasticFullWrap_SideShell_10mm_ReliefStep02Shape",
            plastic_side_rod_relief_shape_defs("GeoOpt_S2_PlasticFullWrap_SideShell_10mm"),
        ),
        (
            "GeoOpt_S2_PlasticFullWrap_BottomCap_10mm",
            plastic_bottom_planes,
            plastic_bottom_z,
            plastic_roles["bottom"] + "; includes two local NF2 outer-support rod relief cutouts",
            "GeoOpt_S2_PlasticFullWrap_BottomCap_10mm_ReliefStep02Shape",
            plastic_bottom_rod_relief_shape_defs("GeoOpt_S2_PlasticFullWrap_BottomCap_10mm"),
        ),
        (
            "GeoOpt_S2_PlasticFullWrap_TopCap_10mm",
            plastic_top_planes,
    ...
    bpe_side_name = "GeoOpt_S2_BPE5_FullWrap_SideShell_SignalWindowCut_20mm"
    bpe_side_vol = pcon_volume(0.0, 360.0, bpe_side_planes)
    volumes.append(
        PatchVolume(
            name=bpe_side_name,
            material="BoratedPolyethylene5wtB",
            shape=f"{bpe_side_name}_ReliefStep02Shape",
            position=(0.0, 0.0, bpe_side_z),
            role=(
                "20 mm borated polyethylene inner side shell with a rectangular negative-x signal-window cutout; "
    ...
    for name, planes, z, role in [
        (
            "GeoOpt_S2_BPE5_FullWrap_BottomCap_20mm",
            bpe_bottom_planes,
            bpe_bottom_z,
            "20 mm borated polyethylene bottom disk inside the plastic skin; extended downward to avoid the retained W baffle and bottom shell",
        ),
        (
            "GeoOpt_S2_BPE5_FullWrap_TopCap_20mm",
            bpe_top_planes,
            bpe_top_z,
            "20 mm borated polyethylene top disk inside the plastic skin, covering the former central service/support opening with local service reliefs",
        ),
    ]:
        vol = pcon_volume(0.0, 360.0, planes)
        shape = bpe_top_shape if name == "GeoOpt_S2_BPE5_FullWrap_TopCap_20mm" else pcon_line(0.0, 360.0, planes)
        shape_defs = bpe_top_shape_defs if name == "GeoOpt_S2_BPE5_FullWrap_TopCap_20mm" else ()
        volumes.append(
            PatchVolume(
                name=name,
                material="BoratedPolyethylene5wtB",
    ...
    volumes.append(
        PatchVolume(
            ...
            role=(
                "5 mm local tungsten bottom gamma baffle retained unchanged from the S1 geometry; "
...
def append_geo_patch(volumes: list[PatchVolume]) -> None:
    ...
    if "BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_PATCH" in text:
        return
    lines = [
        "",
        "// BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_PATCH",
    ...
    lines.append("// END GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_PATCH")
    GEO.write_text(text.rstrip() + "\n" + "\n".join(lines) + "\n", encoding="utf-8")


def append_det_patch(volumes: list[PatchVolume]) -> None:
    text = DET.read_text(encoding="utf-8")
    if "BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_DET" in text:
        return
    lines = [
        "",
        "// BEGIN GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_DET",
        "// S2 plastic-skin draft detector entries.  Transport/scorer validation remains a separate gate.",
    ]
    for vol in volumes:
        if not vol.active:
            continue
    ...
    lines.append("// END GEOOPT_S2_THICK_PLASTIC_BPE_BARREL_DET")
    ...
    OVERLAP_SOURCE.write_text(
        "\n".join(
            [
                ...
                "Run Minimum",
                "Minimum.FileName            /tmp/DelMe_geoopt_s2_thick_plastic_bpe_barrel_overlap",
                "Minimum.NEvents             1",
...
def write_detail_figure(skin: dict[str, float]) -> None:
    ...
    ax_xz.text(-43, 28.5, "cyan: 10 mm full-wrap active plastic scintillator skin", fontsize=8, color="#007f99")
    ax_xz.text(-43, 25.5, "yellow/green: 20 mm borated PE inside plastic", fontsize=8, color="#6f8f00")
    ...
    ax_xz.text(-43, 16.5, "top disks include local service reliefs; BPE bottom extends downward", fontsize=8, color="#007f99")
    ...
    ax_xy.text(-43, 34, f"top disks have local service reliefs; W bottom baffle r={W_BAFFLE_ROUT_CM:.1f} cm", fontsize=8)

    fig.suptitle("GeoOpt S2 thick plastic/BPE barrel draft from Mass_model_511 geometry", fontsize=13)
    fig.tight_layout()
    ...
    README.write_text(
        "\n".join(
            [
                ...
                "## Geometry Choices",
                "",
                f"- Plastic skin: thickness `{PLASTIC_THICKNESS_CM:.3g} cm`, side `r={PLASTIC_RIN_CM:.3g}-{PLASTIC_ROUT_CM:.3g} cm`, `z={PLASTIC_SIDE_ZMIN_CM:.3g}..{PLASTIC_SIDE_ZMAX_CM:.3g} cm`, closed bottom disk and top disk `r=0..{PLASTIC_CAP_ROUT_CM:.3g} cm` with local service reliefs; no phi segmentation.",
                f"- Borated PE: thickness `{BPE_THICKNESS_CM:.3g} cm`, side `r={BPE_RIN_CM:.3g}-{BPE_ROUT_CM:.3g} cm`, `z={BPE_SIDE_ZMIN_CM:.3g}..{BPE_SIDE_ZMAX_CM:.3g} cm`, closed top disk `r=0..{BPE_CAP_ROUT_CM:.3g} cm`.",
                f"- BPE bottom cap: disk `r={BPE_BOTTOM_CAP_RIN_CM:.3g}..{BPE_CAP_ROUT_CM:.3g} cm`, `z={BPE_BOTTOM_ZMIN_CM:.3g}..{BPE_BOTTOM_ZMAX_CM:.3g} cm`; it thickens downward relative to S1 to avoid the unchanged W baffle and bottom shell.",
                f"- Top service reliefs: annular reliefs `{TOP_ANNULAR_RELIEFS}`, CP-Still sector reliefs `{TOP_SECTOR_RELIEFS}`, plus four XS400 rod relief boxes centered at `{TOP_SUPPORT_ROD_RELIEF_CENTERS_CM}`.",
        ...
        json.dumps(
            {
                ...
                "current_revision": {
                    "layer_order_outside_to_inside": [
                        "10 mm active PlasticScintillator full-wrap barrel with top disk and local service reliefs",
                        "20 mm BoratedPolyethylene5wtB barrel with signal-window cutout, top disk, and local service reliefs",
                        "unchanged 5 mm W bottom baffle",
                        "unmodified Mass_model_511 detector",
                    ],
                    "plastic_cm": {
                        "thickness": PLASTIC_THICKNESS_CM,
                        "r_inner": PLASTIC_RIN_CM,
                        "r_outer": PLASTIC_ROUT_CM,
                        "cap_r_inner": TOP_CAP_RIN_CM,
                        "cap_r_outer": PLASTIC_CAP_ROUT_CM,
                        ...
                        "top_large_service_opening_closed": True,
                        "top_annular_reliefs": [list(relief) for relief in TOP_ANNULAR_RELIEFS],
                        "top_support_rod_relief_centers": [list(center) for center in TOP_SUPPORT_ROD_RELIEF_CENTERS_CM],
                        "top_sector_reliefs": [list(relief) for relief in TOP_SECTOR_RELIEFS],
                        "top_support_rod_relief_half_widths": [
                            TOP_SUPPORT_ROD_RELIEF_HALF_X_CM,
                            TOP_SUPPORT_ROD_RELIEF_HALF_Y_CM,
                            TOP_SUPPORT_ROD_RELIEF_EXTRA_HALF_Z_CM,
                        ],
                    ...
                    "borated_polyethylene_cm": {
                        "thickness": BPE_THICKNESS_CM,
                        "r_inner": BPE_RIN_CM,
                        "r_outer": BPE_ROUT_CM,
                        "cap_r_inner": TOP_CAP_RIN_CM,
                        "cap_r_outer": BPE_CAP_ROUT_CM,
                        "side_z_min": BPE_SIDE_ZMIN_CM,
                        "side_z_max": BPE_SIDE_ZMAX_CM,
                        "bottom_z_min": BPE_BOTTOM_ZMIN_CM,
                        "bottom_z_max": BPE_BOTTOM_ZMAX_CM,
                        "bottom_cap_r_inner": BPE_BOTTOM_CAP_RIN_CM,
                        "bottom_cap_r_outer": BPE_CAP_ROUT_CM,
                ...
                "boundaries": [
                    ...
                    "Only the plastic scintillator and borated-polyethylene draft structures are changed relative to S1; the W bottom baffle is retained unchanged.",
                    "The S2 top disks use local reliefs for known service/support conflicts; mechanical clearance still requires review before transport production.",
                    "S2 active veto post-processing must include GeoOpt_S2_PlasticFullWrap* volumes; S1-only active-volume rules will miss this branch.",
                ],
            },
            indent=2,
            sort_keys=True,
