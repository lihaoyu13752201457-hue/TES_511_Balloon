#!/usr/bin/env python3
"""Fail-closed per-shard and checkpoint validator for gamma batch0003."""
...
def _campaign_contract(contract: dict[str, Any], geometry: str) -> dict[str, Any]:
    rows = [row for row in contract.get("campaigns", []) if row.get("geometry") == geometry]
    if len(rows) != 1:
        raise ValueError(f"global contract campaign ambiguity: {geometry}")
    return rows[0]
...
def _expected_job_name(ordinal: int) -> str:
    return f"Background_gamma_fullsphere20_batch0003_shard{ordinal:04d}"
        ...
        with gzip.open(path, "rt", encoding="utf-8", errors="replace") as handle:
            for raw in handle:
                ...
                try:
                    ...
                    continue
                if int(init["particle_type"]) != common.PARTICLE_TYPES[batch.FAMILY]:
                    bad_particle += 1
...
def _light_contract_gate(contract: dict[str, Any], gate: common.Gate) -> dict[str, Any] | None:
    ...
    expected_scope = {
        "included_families": [batch.FAMILY],
    ...
    gate.require(
        stats.get("prior_events_per_geometry") == batch.PRIOR_EVENTS_PER_GEOMETRY,
        "prior event credit mismatch",
        "statistics",
    )
    gate.require(
        stats.get("new_events_per_geometry") == batch.NEW_EVENTS_PER_GEOMETRY,
        "new event target mismatch",
        "statistics",
    )
    gate.require(
        stats.get("shards_per_geometry") == batch.FINAL_SHARD_COUNT,
...
def validate_attempt(
    ...
    label = f"{geometry}/{batch.MODE}/{batch.FAMILY}/shard{ordinal:04d}/attempt{attempt:02d}"
    ...
    campaign = _campaign_contract(contract, geometry)
...
def _full_contract_gate(contract: dict[str, Any], gate: common.Gate) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    for geometry in batch.GEOMETRIES:
        ...
        gate.require(bundle == campaign.get("geometry_bundle"),
                     f"campaign geometry bundle changed: {geometry}", "geometry")
    ...
    try:
        ...
        gate.problem(f"prior 101k physics-input equivalence failed: {exc}", "lineage")
    else:
        gate.require(
...
def _validate_campaign_control_files(
    ...
    campaign_contract = batch.campaign_dir(geometry) / "batch_contract.json"
    normalization = batch.campaign_dir(geometry) / "normalization.json"
    ...
    if campaign_contract.is_file():
        try:
            ...
            expected_campaign = batch._campaign_contract_payload(contract, geometry)
    ...
    if normalization.is_file() and campaign_contract.is_file():
        try:
            ...
            expected_normalization = batch._normalization_payload(contract, geometry)
        except (OSError, UnicodeError, json.JSONDecodeError, KeyError, RuntimeError, ValueError) as exc:
...
def _rehash_prior_gamma_credit(
    prior_ledgers: list[dict[str, Any]],
    gate: common.Gate,
) -> dict[str, Any]:
    """Rebind the credited 101k/geometry to its actual retained artifacts."""
    errors_before = len(gate.errors)
    records: list[dict[str, Any]] = []
    credited: dict[str, int] = {geometry: 0 for geometry in batch.GEOMETRIES}
    for ledger, expected_per_geometry in zip(prior_ledgers[:2], (1_000, 100_000), strict=True):
        gate.require(
        ...
        for geometry in batch.GEOMETRIES:
            try:
                jobs = batch._gamma_jobs(ledger, geometry)
            except (SystemExit, KeyError, TypeError, ValueError) as exc:
                gate.problem(f"{geometry}: cannot enumerate credited gamma jobs: {exc}", "prior_credit")
                continue
            events = sum(int(job.get("events", 0)) for job in jobs)
            gate.require(events == expected_per_geometry,
                         f"{geometry}: prior credited gamma count mismatch in {ledger.get('batch_id')}",
                         "prior_credit")
    ...
    for geometry, events in credited.items():
        gate.require(events == batch.PRIOR_EVENTS_PER_GEOMETRY,
                     f"{geometry}: total prior gamma credit is not 101k", "prior_credit")
    return {
...
def _stage_spec(stage: str) -> tuple[int, int, Path, Path, str]:
    if stage == "5m":
        return (
            batch.STAGE5_SHARD_COUNT,
            batch.STAGE5_TOTAL_EVENTS_PER_GEOMETRY,
            batch.STAGE5_VALIDATION_REPORT,
            batch.STAGE5_LEDGER,
            "PASS__BATCH0003_STAGE5M_MERGE_ELIGIBLE",
        )
    if stage == "10m":
        return (
            batch.FINAL_SHARD_COUNT,
            batch.FINAL_TOTAL_EVENTS_PER_GEOMETRY,
            batch.FINAL_VALIDATION_REPORT,
            batch.FINAL_LEDGER,
            "PASS__BATCH0003_10M_MERGE_ELIGIBLE",
        )
...
def validate_stage(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
    gate = common.Gate()
    end_ordinal, cumulative_target, report_path, _, ledger_status = _stage_spec(stage)
    ...
    planned_seeds = {batch.shard_seed(ordinal) for ordinal in range(1, end_ordinal + 1)}
    prior_ledgers = [_json(path) for path in (batch.BATCH0000_LEDGER, batch.BATCH0001_LEDGER, batch.BATCH0002_LEDGER)]
    gate.require(planned_seeds.isdisjoint(batch._prior_seeds(*prior_ledgers)),
                 "stage seed registry overlaps prior batches", "seeds")
    prior_credit_revalidation_initial = _rehash_prior_gamma_credit(prior_ledgers, gate)

    campaigns: list[dict[str, Any]] = []
    initial_input_digests: dict[str, str] = {}
    for geometry in batch.GEOMETRIES:
        try:
            current_input_digest = batch._verify_attempt_inputs(contract, geometry, environment)
        except Exception as exc:
        ...
        for ordinal in range(1, end_ordinal + 1):
            ...
            gate.require(pair.get("ordinal") == ordinal
        ...
        expected_new = cumulative_target - batch.PRIOR_EVENTS_PER_GEOMETRY
        gate.require(len(jobs) == end_ordinal, f"{geometry}: validated shard count mismatch", "campaigns")
        gate.require(new_events == expected_new, f"{geometry}: new event count mismatch", "campaigns")
        gate.require(len({int(job["seed"]) for job in jobs}) == end_ordinal,
                     f"{geometry}: duplicate stage seed", "seeds")
        try:
            current_input_digest_post = batch._verify_attempt_inputs(contract, geometry, environment)
        except Exception as exc:
        ...
        campaign_contract, normalization = _validate_campaign_control_files(contract, geometry, gate)
        campaigns.append({
            "geometry": geometry,
            "mode": batch.MODE,
            "family": batch.FAMILY,
    ...
    try:
        ...
        for geometry in batch.GEOMETRIES:
            try:
                final_digest = batch._verify_attempt_inputs(contract, geometry, final_environment)
            except Exception as exc:
            ...
            gate.require(final_digest == initial_input_digests.get(geometry),
                         ...
                         "input_integrity")
    prior_credit_revalidation = _rehash_prior_gamma_credit(prior_ledgers, gate)
    gate.require(prior_credit_revalidation == prior_credit_revalidation_initial,
                 "prior 101k credited artifacts drifted during stage validation", "prior_credit")
    status = "PASS" if not gate.errors else "FAIL"
    report = {
        "schema_version": 1,
        "status": status,
...
def _write_stage(stage: str, *, check: bool) -> int:
    _, _, report_path, ledger_path, _ = _stage_spec(stage)
    ...
    if report_path.exists():
        if _json(report_path) != report:
            raise SystemExit(f"existing batch0003 {stage} PASS report differs from revalidation")
    else:
    ...
    if ledger_path.exists():
        if _json(ledger_path) != ledger:
            raise SystemExit(f"existing batch0003 {stage} ledger differs from revalidation")
    else:
...
def main() -> int:
    ...
    mode.add_argument("--stage", choices=("5m", "10m"), help="validate a complete checkpoint")
