#!/usr/bin/env python3
"""Build SH3 OptV3 compact event/deposit catalog.

Self-contained (no package 58 dependency).  Streams accepted receipts:
  - 22 INSTANT + 19 BUILDUP corrected per round x 4 rounds = 164 corrected jobs
  - 33 delayed shards (8 M triggers)
...
taken from the IA INIT ZA field (SH3 delayed cards seed the exact production
position and parent, so IA INIT ZA is authoritative for the delayed lineage).
Prompt has ZA = -1.
"""
...
def topology_keep(measured_hits: list[Any]) -> bool:
    """Simplified Step05 side-entry keep rule.

    For 6-TES stacked SH3 geometry the retained rule in
    build_v3p5_centerfinger_step05_l1_response.py is: single-hit keep, zero
    reject, >6-hit keep, else keep if the Compton scatter sequence intersects
    the retained side-entry disk.  Without the disk-intersection primitive we
    keep multi-hit events (rejection is the retained fallback for MAX_ENUM_HITS
    exceeded); this is the conservative default used by the reject_policy_for_more_than_six_hits
    = "keep" clause in analysis_inputs.  Downstream we compare to a stricter
    single-hit-only variant below.
    """
...
def flags_for_event(
    ...
    topo = combined and topology_keep(measured_hits)
    bits_broad = 0
...
def prepare_jobs(cfg: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for round_name in cfg["campaigns"]["corrected_rounds"]:
        rec_dir = corrected_root / "rounds" / round_name / "corrected/run/receipts"
...
def scan_job(job: dict[str, Any], cache_dir_text: str, threshold_keV: float) -> dict[str, Any]:
    ...
    def flush() -> None:
        nonlocal current_id, current_za, init_count, pixels, plastic_keV, bgo_keV
        if current_id is None:
            return
        resolved_za = -1
        if job["stream"] == "delayed":
            if init_count != 1 or current_za is None:
                # Delayed source cards emit exactly one primary per event with a
                # parent-ZA label; if a shard breaks this contract we cannot
                # assign lineage, but rather than abort we bucket it as -1 (a
                # diagnostic sentinel).  Below we count za=-1 events.
                resolved_za = -1
            else:
                resolved_za = int(current_za)
        ...
        for uid, record in sorted(pixels.items()):
            ...
            if measured >= PIXEL_THRESHOLD_KEV:
                measured_hits.append(SimpleNamespace(e=measured, x=x, y=y, z=z))
...
def main() -> int:
    ...
    with ProcessPoolExecutor(max_workers=max(1, args.workers)) as executor:
        futures = {executor.submit(scan_job, job, str(CACHE), threshold): job for job in jobs}
    ...
    summary = {
        "status": "PASS__SH3_OPTV3_COMPACT_EVENT_CATALOG",
        ...
        "normalization": {
            "prompt_rule": "instant weight = 1/sum(TT_family) over all accepted instant receipts (4 rounds)",
