...
5. The activation chain now has an analytic `I-128` CsI anchor added
   2026-06-11 (A5): chain `8.185 Bq/kg` vs two-group analytic `6.323 Bq/kg`,
   ratio `1.29`. Literature citations for published CsI instrument/irradiation
   comparisons are still TODO slots.
...
Status: present for DEMO2 detector/ADR/passive-mass model.
...
into the legacy compatibility path used by the simulation chain. The current
authority geometry is
`outputs/geometry/XZTES_ADR_v4c_mkflange_cm/TibetTES_ADR_v4c_mkflange_cm.geo.setup`.
The path still has old `v4c/cm` naming, but the contents are DEMO2:
TES pixels, CsI active shield, staged Al thermal windows, Be entrance window,
Cu/W passive shield, Cryoperm/ADR/passive service proxies.
...
| Validator | `outputs/reports/validation_new_geo_re/validation_new_geo_re.json` |
| Detector geometry | `outputs/geometry/XZTES_ADR_v4c_mkflange_cm/TibetTES_ADR_v4c_mkflange_cm.geo.setup` |
| Geometry visual outputs | `stepwise_maintenance/outputs/geometry/` |
| Geometry generator | `code/geometry/GenerateGeo_ADR_v4c_mkflange.py` |
| EXPACS prompt/buildup runner | `code/tools/run_equiv2602_pipeline_NEW_GEO.py` |
| Delayed source audit | `stepwise_maintenance/step03_delay_source/outputs/delay_source_audit.json` |
| Delayed source sampled map | `stepwise_maintenance/step03_delay_source/outputs/delay_source_2d_schematic.png` |
| RPIP production-position map | `stepwise_maintenance/step03_delay_source/outputs/rpip_production_map.png` |
| Day-15 time-axis catalog | `outputs/reports/day15_complete_report/complete_day15_summary.json` |
| Active shield and Compton/FoV logic | `code/tools/make_complete_day15_report_ADR.py` |
| Compton classifier internals | `code/tools/make_day15_report_ADR.py` |
| Optics authority | `stepwise_maintenance/step04_opticsim/optics_aeff_authority.json` |
| EventList bridge | `stepwise_maintenance/step09_optics_bridge/outputs/step09_optics_bridge_summary.json` |
| Detector-coupled focus response | `stepwise_maintenance/step09_optics_bridge/outputs/detector_coupled_focus_response.json` |
| Time variation | `stepwise_maintenance/step06_mission_time_variation/outputs/step06_mission_time_variation_summary.json` |
| Source cases | `stepwise_maintenance/step07_source_cases/outputs/source_case_summary.json` |
| Significance | `stepwise_maintenance/step08_significance/outputs/step08_significance_summary.json` |
| Headline statistical uncertainties | `stepwise_maintenance/step08_significance/outputs/headline_statistical_uncertainty.csv` |
| CsI activation analytic anchor | `stepwise_maintenance/step03_delay_source/outputs/csi_activation_anchor_20260611.md` |
