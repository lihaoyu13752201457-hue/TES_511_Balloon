...
通过详细质量模型的探测器耦合 Monte Carlo 输运，是预测伽马射线谱仪响应和本底的成熟方法 \cite{Sturner2003}。本节描述从源生成到飞行性能评估的端到端模拟链。聚焦光学与探测器系统由相互独立的质量模型表征。光学分支在 Laue 透镜模型中输运目标点源，并记录到达焦平面的光子相空间。探测器侧则在第~\ref{sec:geometry}~节的探测器--低温系统质量模型中输运四类独立构建的输入：由焦面聚焦光子相空间构建的 EventList、宽带瞬发大气粒子场、按记录产生位置抽样的延迟衰变，以及由文献测量值约束并作为瞬发本底独立输运的单能大气 511\,keV 线源。四条计算流统一施加探测器响应和 511\,keV 能窗选择，随后进行主动 veto 与 Compton 拓扑检验。聚焦、宽带瞬发和延迟事例目录的显式公共时间轴用于检验泊松符合分组；最终任务折叠还在每个轨迹时间箱中纳入大气线率和探测器占有率。选后计算流给出信号率和分量本底率，并用于选后本底来源分解和任务时间--轨迹折叠。这两项分析共同为最终屏蔽几何优化与方案比较提供依据；统一的响应和事例选择约定使后续比较保持一致口径。

图~\ref{fig:workflow} 将该模拟链组织为三条物理分支：聚焦信号、瞬发外部大气本底和延迟活化本底。在瞬发分支内，EXPACS/PARMA 宽带粒子场与基于文献测量值约束的单能大气 511\,keV 线分别在完整的探测器--低温系统质量模型中独立输运。所得事例目录统一施加探测器响应、主动 veto 和 Compton 拓扑检验；选后率随后进入本底来源分解和任务时间--轨迹折叠，并由所得来源与任务性能信息指导最终屏蔽几何优化和方案比较。下文逐层说明各阶段的输入、操作与输出。

日时间尺度的飞行会采样连续的海拔与地磁状态，单次 Monte Carlo 快照不能按时间线性外推。任务时间轨迹折叠的动机、解析定义、多点输运检验与性能曲线见第~\ref{sec:mission_time_fold}~小节。

\begin{landscape}
\centering
\vspace*{\fill}
\includegraphics[width=0.94\linewidth]{paper_source_figure_table/fig_simulation_workflow_m04_zh_20260810.pdf}
\captionof{figure}{端到端模拟与分析流程。聚焦信号、两条瞬发大气本底计算流和延迟活化本底分别输运，随后统一施加探测器响应、主动 veto 和 Compton 拓扑检验。选后率进入本底来源分解和任务时间--轨迹折叠，并共同用于最终屏蔽几何优化和方案比较。}
\label{fig:workflow}
\vspace*{\fill}
\end{landscape}
...
在第~\ref{sec:optics} 节定义聚焦光学后，本文独立构建四个输运分量：入射载荷的瞬发大气粒子场、仪器内积累的延迟放射性群体、单能大气 511\,keV 线以及参考信号点源。活化产生与延迟衰变输运仍作为两个阶段处理。该构建方式在施加共同探测器响应和事例选择前，保留了各分量独立的归一化与来源追溯关系。
