...
本文模拟的仪器面向既有观测尚未探测到的银河系中心 511 keV 点源或未分辨中央增强成分。现有 INTEGRAL/SPI 测量已经建立了以弥散核球和银盘为主的 511 keV 天图，并把叠加在弥散成分之上的点状 511 keV 源限制到约 $10^{-4}\phcms$ 的通量量级；基于 INTEGRAL/IBIS 的紧凑源搜索也未发现银河系 511 keV 点源，但在银河中心约 10 Ms 曝光下给出约 $1.6\times10^{-4}\phcms$ 的上限 \cite{Knodlseder2005AllSky,DeCesare2011}。若取气球飞行海拔处 511 keV 光子的典型大气透过率 $T\simeq0.739$，该上限对应的载荷上方光子流量尺度约为 $1.2\times10^{-4}\phcms$。本文因此采用 $\fzero=10^{-4}\phcms$ 作为参考点源光子通量尺度。和 INTEGRAL 的宽视场谱学/编码孔径搜索相比，本文模拟的方案引入 Laue 光学聚焦以提高点源响应，并采用高能量分辨率、多层厚吸收体 TES 微量热计阵列作为焦平面，以增强对 511 keV 窄线的谱学判别能力 \cite{Shirazi2023,Caputo2017}。

本工作首先考虑早期测试用的气球载荷平台，并期望后续拓展到卫星观测平台。

这一目标对仪器和模拟提出四方面要求：其一，望远镜须将 511 keV 光子汇聚到小焦平面，从而在不牺牲收集面积的前提下压低探测器相关本底。其二，焦平面须具备亚 keV 能量响应，使线窗本底随窄谱窗、而非宽 MeV 连续谱缩放。其三，探测器与屏蔽模型须能抑制大气、活化和侧向入射本底，并显式计入 TES 吸收体附近的被动材料。其四，模拟链须一致处理瞬发本底、延迟活化、聚焦信号、反符合 veto、事例拓扑与任务时间变化，使信号与本底在同一套探测器选择下评估。
...
TES 探测器概念采用本实验室研发的 AlMn 薄膜耦合吸收体结构；AlMn 合金的优势包括相变温度可通过成分和退火工艺调节、便于阵列制备，以及相对较低的磁场敏感性 \cite{Xie2026AlMn,Vavagiakis2017AlMnMagnetic}。在本文概念设计中，TES 温度计膜为约 $200\,\mathrm{nm}$ 量级 AlMn 薄膜，而 $\gamma$ 射线吸收体为 mm 量级金属体。吸收体采用 $1.5\times1.5\times3.0\,\mathrm{mm^3}$ Ta 像素；Ta 的高原子序数提高 511 keV 光子的相互作用概率，$3.0\,\mathrm{mm}$ 厚度在 511 keV 处对应约 $49\%$ 的单层相互作用概率，6 层阵列可提供接近归一的几何相互作用概率。当前几何中相邻 Ta 阵列层中心距为 $12\,\mathrm{mm}$，这一间距用于保留后文康普顿运动学 veto 所需的多层命中拓扑。选择 Ta 还考虑了该体积可提供足够高的饱和能量、低温低比热（本文采用 $10^{-2}\,\mathrm{J\,K^{-1}\,m^{-3}}$ 的量级估计）和较高的光电--康普顿相互作用比；按 NIST XCOM 在 511 keV 附近的截面估算，该比例约为 0.731 \cite{NISTXCOM}。根据实验室经验，以及将当前 Ta 吸收体热容 $67.5\,\mathrm{pJ/K}$ 带入，在 511 keV 处约取 $\Delta E_{\mathrm{FWHM}}\simeq420\,\mathrm{eV}$ 作为当前模拟能力分辨率。

由于 TES 薄膜与 Ta 吸收体在尺寸和质量上相差悬殊，且 TES 膜层本身不贡献显著的 511 keV 相互作用，本文在输运几何中用 Ta 吸收体代表 TES 像素的主要敏感体积。显式敏感体积为 6 层、每层 376 个 Ta 吸收体像素，总计 2256 个像素，1.5mm长宽 3 mm厚度。每层 Ta 像素下方放置 Si 衬底代理，用 Si 代表衬底；这样也保留了硅在中子输运中可能贡献的弹性散射贡献。TES 膜层、布线和读出细节不作为显式相互作用体积逐层建出，而是通过假设能量响应和服务质量代理进入模型。本文在当前稿件中以 TES 能量分辨率作为 511 keV 线窗宽度，取 W511 = 511 keV ± 420 eV，即 510.58–511.42 keV。
...
本节描述从源生成到飞行性能评估的整条端到端模拟链。信号与本底并不共用同一几何，而是各自在独立的质量模型中起步：信号支在 10 m 上游的 Laue 光学模型里把 511 keV 光子聚焦到焦平面，记录穿过 Be 入射孔径的光子；本底支则在第~\ref{sec:geometry}~节的探测器/低温系统质量模型中直接输运瞬发大气粒子，以及活化产物按记录产生位置生成的延迟衰变。两支在探测器模型里才汇合：聚焦光子被重放进同一探测器/低温系统模型，与本底一道经过同一套探测器响应与 veto，随后依次进入详细本底分析、任务时间与轨迹折叠，以及飞行性能评估与设计优化建议。正因为聚焦信号与两路本底在任何本底分解或灵敏度陈述之前都通过了这同一套响应和 veto，后续结果才落在一致的事例选择之上。

通过详细质量模型的探测器耦合 Monte Carlo 输运，是预测伽马射线谱仪响应和本底的成熟方法 \cite{Sturner2003}；图~\ref{fig:workflow} 把这条链路按计算层级展开：几何定义、瞬发与活化积累输运、延迟源构建、聚焦光学输运、探测器耦合信号重放、veto 机制、本底细节分析、时间/飞行轨迹变化，以及飞行性能评估与设计优化意见——下文逐层说明各层的输入、操作与输出。本文以产生位置采样延迟衰变作为主要延迟源表示：延迟衰变从记录到的放射性产生位置采样，而不是重新分配到轴对称径向分布。
...
这里我们采用的信号光子以一个SPI曾经探测到过的V404天体的点源作为参考 \cite{Siegert2016V404}，V404是银河系盘面的瞬态点源，这里只做参考，以megalib的远场源方式构建 \cite{Zoglauer2006}，\textcolor{red}{详细构建方式见3.2.3点源构建部分}。除了信号引入，我们还引入了宇宙线瞬时本底成分，也是以远场源方式构建，也引入了活化本底的构建。\textcolor{red}{将上述三种源的输运分别模拟，三次模拟中，均截取穿过焦平面所在平面的粒子通量，种类，能量作为输出，把三次模拟输出以等效观测时间为基准归一化作为总输出，该总输出可以单独作为一个平行束源输入到探测器的质量模型中。}

\subsection{瞬发大气与延迟活化源模型}
\label{sec:background}

在第~\ref{sec:optics} 节定义了聚焦信号之后，本底一侧还需要两层源：入射到
载荷上的瞬发大气粒子场，以及该粒子场在仪器中积累起来的延迟放射性群体。两者分开
构建，使源的产生过程不吸收本属于探测器选择的假设：本节定义的都是入射粒子或衰变
核素，veto、拓扑筛选和线窗都推迟到第~\ref{sec:selection} 节才施加。
...
活化产生流不作为即时探测器本底计数，而是记录每个放射性产物的核素、材料体积和产生
位置。这些记录通过产生--衰变积累方程转换为固定第 15 天放射性群体，基态半衰期已对照
NUBASE2020 校验 \cite{Kondev2021NUBASE}。对核素 $k$，
\begin{equation}
...
构建另在 Geant4/MEGAlib 的 stepping-action 层加入自定义记录钩子：在活化积累模式中
...
本文给出了气球载指向式 511 keV TES Laue 透镜望远镜的探测器耦合本底和灵敏度研究。参考计算把 Ge(111) Laue 光学响应与 TES 焦平面质量模型、瞬发大气输运、延迟活化、主动屏蔽 veto、拓扑/视场选择、任务时间率折叠和计数显著度评估结合起来。在主要 $510.58$--$511.42\keV$ 未分辨线窗中，第 15 天选后率为 $B\simeq3.92\times10^{-2}\cps$，对 $10^{-4}\phcms$ 参考点源有 $S\simeq1.19\times10^{-3}\cps$；任务时间折叠后得到 $Z_{20\mathrm{d}}\simeq7.80$ 和 $\fthree(20\mathrm{d})\simeq3.85\times10^{-5}\phcms$。该结果为继续评估聚焦式 TES 谱学作为 511 keV 紧凑源和未分辨中央增强搜索的互补技术提供了探测器耦合选后率依据，同时指出瞬发大气本底抑制、延迟活化系统误差验证和重建验证是下一阶段的主要验证任务。

\appendix
\setcounter{figure}{0}

% [RECOVERY GAP: original lines 647-683 not found in logs]
