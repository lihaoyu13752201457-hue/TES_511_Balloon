...
Detector-coupled Monte Carlo transport through detailed mass models is a mature method for predicting the response and background of gamma-ray spectrometers \cite{Sturner2003}. This section describes the end-to-end chain from source generation to flight-performance assessment. The focusing optics and detector system are represented by independent mass models. In the optics branch, the target point source is transported through the Laue-lens model, and the phase space of photons reaching the focal plane is recorded. On the detector side, four independently constructed inputs are transported through the detector--cryostat mass model of Section~\ref{sec:geometry}: the focal-plane EventList constructed from the focused-photon phase space, the broadband prompt atmospheric field, delayed decays sampled at their recorded production positions, and a monoenergetic atmospheric 511\,keV prompt-background source constrained by published measurements. The four streams receive the same detector response and 511\,keV energy-window selection, followed by the active veto and Compton-topology test. An explicit common-time-axis realization of the focused, broadband-prompt, and delayed catalogues validates the Poisson coincidence grouping; the final mission fold additionally includes the atmospheric-line rate and detector occupancy in each trajectory bin. The selected streams provide the signal rate and component-resolved background rates used for background-source decomposition and mission time--trajectory folding. These two analyses jointly inform the final shielding-geometry optimization and design comparison, while the common response and selection contract keeps all subsequent comparisons on a consistent basis.

Figure~\ref{fig:workflow} organizes the chain into three physical branches: focused signal, prompt external atmospheric background, and delayed activation background. Within the prompt branch, the EXPACS/PARMA broadband field and the literature-constrained monoenergetic atmospheric 511\,keV line are transported independently through the complete detector--cryostat mass model. The resulting event catalogues are subjected to the same detector response, active veto, and Compton-topology test before the selected rates enter background-source decomposition and mission time--trajectory folding. The resulting source and mission-performance information is then used for the final shielding-geometry optimization and design comparison. The following subsections describe the inputs, operations, and outputs of these stages.
...
\captionof{figure}{End-to-end simulation and analysis workflow. The focused signal, two prompt atmospheric streams, and delayed activation background are transported separately and then subjected to a common detector response, active veto, and Compton-topology test. The selected rates enter background-source decomposition and mission time--trajectory folding, which jointly inform the final shielding-geometry optimization and design comparison.}
\label{fig:workflow}
\vspace*{\fill}
\end{landscape}
...
After the focusing optics have been defined in Section~\ref{sec:optics}, four
transport components are constructed independently: the prompt atmospheric
particle field incident on the payload, the delayed radioactive population
accumulated in the instrument, a monochromatic atmospheric 511\,keV line, and the
reference signal point source. Activation production and delayed-decay transport
remain separate stages. This construction preserves the normalization and
provenance of each component before the common detector response and event
