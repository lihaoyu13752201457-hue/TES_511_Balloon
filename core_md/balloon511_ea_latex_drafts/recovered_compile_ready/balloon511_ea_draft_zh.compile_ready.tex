...
\end{equation}
并以概率 $r_{kj}/R_k$ 有放回地选出记录，在参考观测窗 $[0,T]$ 内为每条记录赋以均匀随机时刻；将三条流的全部事例合并后按时刻排序，即得到一条同时承载三流物理率的探测器时间轴。相邻时间间隔不超过 $\tau=1\,\mu\mathrm{s}$ 的事例归并为一个符合候选组 $c$。在本文基准中（图~\ref{fig:s33_poisson}），叠加于该时间轴的三条流全能段探测器率约为：瞬发本底 $676\,\mathrm{Hz}$、延迟活化 $71\,\mathrm{Hz}$、聚焦信号 $0.99\,\mathrm{Hz}$，故符合时间轴由瞬发本底主导；参考曝光内约 $8.7\times10^{6}$ 个事例实例仅产生 $1134$ 个跨流偶然符合，偶然符合活时间因子 $\simeq0.9992$（线窗与屏蔽阈值在其后作用于这些符合候选组）。三条流归一后的求和能谱见图~\ref{fig:s33_spec_norm}：该窗线主导，连续谱饥饿。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{paper_source_figure_table/fig_s33_poisson_normalization.png}
\caption{率归一示意。(a) 瞬发、延迟与聚焦信号三条流按各自全能段符合轴率作为独立齐次泊松过程抽样，合并并按时刻排序到同一时间轴；(b) $\tau=1\,\mu\mathrm{s}$ 内的事例构成一个符合候选组，图中标注了参考曝光内的偶然符合记账（基准值）。(a) 中的事例位置为示意性泊松实现。}
...
\end{equation}
其全宽等于第~\ref{sec:geometry} 节质量模型采用的 TES 能量分辨率 $\Delta E_{\mathrm{FWHM}}\simeq420\,\mathrm{eV}$，在保留主要线响应的同时抑制连续谱本底。若同时满足
\begin{equation}
  E_{\mathrm{sh}}^{(c)} < E_{\mathrm{veto}}, \qquad E_{\mathrm{veto}}=50\keV,
\end{equation}
则通过主动屏蔽，其中屏蔽能量在基准 CsI 配置下由 CsI 主动屏蔽体积于后处理读出。该层是率级本底压低的主要来源。在本文基准中，该门把瞬发本底从 $161$ 个事例（$0.1188\cps$）压到 $60$ 个（$0.0407\cps$，存活 $34.3\%$）、延迟活化从 $54$ 压到 $33$（存活 $61.1\%$），而聚焦信号 $30323$ 个事例全部保留（屏蔽零沉积，存活 $100\%$）；详见表~\ref{tab:phase2_cutflow}。其对背景能谱的作用见图~\ref{fig:s33_spec_anti}。
...
\end{equation}
取最小者为最优次序（残差并列时取首散射杠杆臂最长者），再对其首两点施加同一“81 轨迹—窗盘相交”检验。凡无任何有效次序者归为未重建类。在本文基准中，通过主动屏蔽的事例经拓扑检验分为单点、通过视场的重建多点与否决三类：瞬发 $60=37$ 单点 $+\,17$ 重建多点 $+\,6$ 否决，延迟 $33=19+11+3$，聚焦信号 $30323=17663$ 单点 $+\,12051$ 重建多点 $+\,608$ 否决 $+\,1$ 未重建保留，故三流选后分别为 $54$、$30$、$29715$ 个事例。在现存 Mass\_model\_511 目录上的重导出（图~\ref{fig:s33_compton_mult}）按 TES 多重度把选后聚焦信号组分为 $17598$ 单点、$9794$ 双命中、$2340$ 三命中及以上（其中 $370$ 双命中与 $262$ 三命中及以上被康普顿否决），而瞬发与延迟的多命中数很小（双/三命中及以上分别为 $25/7$ 与 $12/3$）。
...
未重建类在基准选后率中予以保留，但不计作通过视场的事例，因此本文线窗结果并未从剔除歧义重建失败中获取灵敏度。该策略并非净灵敏度的保守下界：诊断性替代选择显示，若改用仅保留单像素事例的口径（将全部多命中候选作为 veto 丢弃），在当前 $\wii$ 样本中将保留 $59.4\%$ 的聚焦信号（$17663/29715$）与约 $66.7\%$ 的选后本底，把简单 $S/\sqrt{B}$ 降至基准值的约 $0.73$。因此，基准选择把歧义多命中事例视为一个需要后续重建验证的模型边界，而非把激进事例拒绝作为灵敏度增益来源。
