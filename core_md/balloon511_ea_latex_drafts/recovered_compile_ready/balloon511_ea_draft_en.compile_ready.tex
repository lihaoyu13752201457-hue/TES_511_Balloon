...
\begin{equation}
  \wii = [510.79,511.21]\keV .
\end{equation}
The first selection layer is the active-shield veto. Candidate events are
grouped on a common Poisson time axis and are rejected if the summed CsI shield
energy in the coincidence window exceeds the analysis threshold. For the
baseline CsI configuration the veto threshold is $50\keV$ and the coincidence
window is $1\,\mu\mathrm{s}$. The shield sum is evaluated from the CsI
