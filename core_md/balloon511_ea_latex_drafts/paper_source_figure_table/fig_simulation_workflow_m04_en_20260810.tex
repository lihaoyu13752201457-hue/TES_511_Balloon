...
\node[promptbox, text width=3.90cm] (expacs) at (4.40,-5.25)
  {EXPACS/PARMA\\atmospheric $\gamma$-ray\\continuum and prompt\\particle fields};
\node[promptbox, text width=3.60cm] (prompttr) at (9.10,-5.25)
  {PROMPT direct\\prompt-background\\transport};
...
\node[promptbox, text width=4.10cm] (linesrc) at (4.40,-7.95)
  {Atmospheric positron-annihilation\\511 keV line source\\constrained by\\published\\measurements};
\node[promptbox, text width=3.80cm] (linetr) at (9.20,-7.95)
  {Independent transport of a\\monoenergetic source for the\\atmospheric 511 keV\\prompt background};
\node[eventbox=prompt, text width=2.40cm] (lineevt) at (17.70,-7.95)
  {Atmospheric 511 keV\\prompt photon event\\catalogue};
...
\node[delayedbox, text width=3.60cm] (buildup) at (9.10,-10.65)
  {BUILDUP activation-\\production transport};
...
\draw[flow] (lineevt.east) -- (19.35,-7.95);
...
\node[sharedbox, text width=5.55cm, minimum height=1.30cm] (geometry)
  at (25.10,-15.65)
  {Final-design assessment and\\constraints for subsequent\\shield optimization};
