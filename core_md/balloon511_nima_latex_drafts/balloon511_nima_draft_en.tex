...
\(\rightarrow\)
\flowbox{Delayed inventory\\Step03}
\(\rightarrow\)
...
\(\rightarrow\)
\flowbox{EventList bridge\\Step09}
\(\rightarrow\)
...
The numerical hierarchy used below follows the same audit logic. Day-15
selected rates are quoted from the Step05 physical-reference-flux block after
the unit EventList signal stream has been scaled by the f10m A1 effective area
and the reference atmospheric transmission. Mission-mean rates are quoted from
Step06, and the headline $Z$, $T_{3\sigma}$, and flux thresholds are quoted
from Step08. Exact-position support-size claims are quoted only after the
sampled source has been transported through Step05--Step08. This separation is
used to avoid mixing unit-injection bookkeeping rates, day-15 direct
expectations, and mission-folded sensitivities in a single headline.
...
The detector core is represented by six layers of 376 TES pixel copies
($2256$ copies in total). The side-entry collimator is therefore an engineering
aperture proxy rather than a pixel-matched 376-hole collimator. The geometry
closure fixes these material and coordinate choices before rate calculations
are made; it does not by itself define a sensitivity.

The active shield is treated quantitatively by post-processing rather than by relying on native production-trigger filtering. Candidate events are grouped on a common Poisson time axis and vetoed if the summed CsI shield energy in the coincidence window exceeds the analysis threshold. For the current CsI branch the active-veto threshold is $50\keV$ and the coincidence window is $1\,\mu\mathrm{s}$. This choice avoids filtering activation-production storage at the transport stage and keeps the veto definition common to prompt, delayed, and focused-science streams.
...
The optics and detector are coupled through an EventList bridge. Focal-crossing rows accepted inside the Be-window gate are transformed into the local detector coordinate system and replayed through the full detector/cryostat geometry. This avoids using an analytic phase-space projection as the signal authority and preserves the optical focusing handoff at the detector entrance plane. The v3p5 bridge transported 37,194/37,194 EventList rows. The bridge is a signal handoff only: it does not add cosmic-ray prompt background or delayed activation in the Laue-lens hardware. At $\fref=10^{-4}\cms$, the Step05 hard-window signal response is $0.00118117\cps$, corresponding to a Step07 response of $11.8117\,\mathrm{cps}/(\mathrm{ph\,cm^{-2}\,s^{-1}})$.

\section{Prompt and delayed background model}
\label{sec:background}

The background model contains prompt atmospheric transport and delayed activation. The full-stat prompt stream and the activation-buildup stream each generated 25,210,216 particles. The prompt input is split into 68 SIM/DAT products, and each primary-particle tag is normalized with its own transported exposure using the rule $r_{\mathrm{event}}=1/\sum TT_{\mathrm{tag}}$. This per-tag normalization is important because gamma and non-gamma prompt streams have different splitting and exposure structure. Delayed activation is constructed from the activation-production ledgers and corrected ground-state half-lives. The fixed day-15 delayed inventory activity for the current chain is $85.636696\,\mathrm{Bq}$.

The main methodological upgrade in the current branch is exact-position delayed-source transport. Instead of compressing production positions into axisymmetric radial-profile source blocks, the exact-position chain samples equal-flux \texttt{PointSource} support blocks from weighted radioactive-production rows. The baseline exact-position source uses $M=5000$ support blocks drawn from 253,770 sampling rows with seed 260613, contains no \texttt{RadialProfileBeam} blocks, and preserves the fixed activity budget. The delayed transport used $10^6$ triggers and stored $10^6$ events, with observation time $11530.473845\,\mathrm{s}$.
...
The Step05 summary also records a day-15 direct expectation. The manuscript
headline does not use that constant-rate direct value; it uses the Step06
mission-time fold and the Step08 cumulative significance. The distinction is
small for the present hard-window case, but keeping it explicit is necessary
for comparing branches and sidecars.
...
The BGO branch was propagated through the same hard-window Step05--Step08
rate chain as the CsI exact-position authority. It includes full-stat prompt
and buildup transport, a 5000-block exact-position delayed source drawn from
true RPIP rows, delayed transport with 1,000,000 stored events, and detector
replay of the f10m A1 focused EventList. Table~\ref{tab:bgo_control}
summarizes the matched comparison. Relative to the CsI exact-position chain,
the BGO sample lowers the W2 mission-mean background by 7.738\%, raises the
20-d significance by 4.541\%, and lowers the 20-d $3\sigma$ flux threshold by
4.344\%. This is a hard-window material-control result only; no BGO
spatial/profile-likelihood sidecar or threshold/material-uncertainty scan is
included.

\begin{table}[t]
\centering
\caption{Matched CsI and equal-attenuation BGO hard-window comparison.}
...
\midrule
Mission-mean $B_{\wii}$ (cps) & 0.0626835 & 0.0578330 & $-7.738\%$ \\
Mission-mean $S_{\wii}$ at $\fref$ (cps) & 0.00117281 & 0.00117756 & $+0.405\%$ \\
