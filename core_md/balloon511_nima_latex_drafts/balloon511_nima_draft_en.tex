...
Four requirements follow. First, the telescope must concentrate 511 keV photons so that a small focal plane can be used without sacrificing the collecting area. Second, the focal plane must provide sub-keV energy resolution so that the line-window background scales with a narrow spectral interval rather than a broad MeV-band continuum. Third, the detector and shield design must suppress atmospheric, activation, and side-entry backgrounds without placing excessive passive material near the TES absorbers. Fourth, the simulation chain must treat prompt background, delayed activation, focused signal, vetoes, event topology, and mission-time variation consistently. These requirements define the methods used below.
...
\begin{table}[t]
\centering
\caption{Primary sensitivity and selected comparison cases. The exact-position hard-window row is the current rate authority. The radial-profile row is a conservative baseline cross-check. Spatial and line-of-sight rows are sidecars, not replacements for the hard-window authority.}
\label{tab:headline}
\begin{tabular}{lccccc}
\toprule
Case & $B_{\wii}$ (cps) & $S_{\wii}$ at $\fref$ (cps) & $Z_{20\mathrm{d}}$ & $T_{3\sigma}$ (d) & $\fthree(20\mathrm{d})$ \\
\midrule
Exact-position hard window & 0.0624651 & 0.00118117 & 6.15522 & 4.73758 & $4.87391\times10^{-5}$ \\
Radial-profile baseline & 0.0729576 & 0.00118117 & 5.70221 & 5.46687 & $5.26111\times10^{-5}$ \\
Focused $r_{90}$ sidecar & 0.0232510 & $\approx0.001063$ & 8.17566 & 2.23643 & $3.66943\times10^{-5}$ \\
45$^\circ$ LOS sidecar & -- & -- & 5.02544 & -- & $5.96962\times10^{-5}$ \\
Multi-annulus sidecar & -- & -- & 8.45804 & -- & $3.54692\times10^{-5}$ \\
\bottomrule
\end{tabular}
\end{table}

The hard-window result is intentionally conservative relative to the spatial sidecars. It requires only the narrow energy window, active-shield veto, and topology/FoV consistency selection. The $r_{90}$ sidecar illustrates the possible gain from using the focused spot in the local side-window detector frame, but it should be treated as a post-processing spatial analysis rather than a finalized profile-likelihood result.
...
\begin{tabular}{lcccc}
\toprule
Label & $M$ & Seed & $B_{\wii}$ (cps) & $Z_{20\mathrm{d}}$ \\
\midrule
\texttt{fullstat\_v2\_exactpos} & 5000 & 260613 & 0.0624651 & 6.15522 \\
\texttt{m05000\_s260614} & 5000 & 260614 & 0.0631683 & 6.12141 \\
\texttt{m20000\_s260613} & 20000 & 260613 & 0.0627261 & 6.14261 \\
\texttt{m50000\_s260613} & 50000 & 260613 & 0.0629804 & 6.13039 \\
\bottomrule
\end{tabular}
\end{table}
