...
\title{气球载 511 keV Laue 透镜 TES 望远镜本底和未分辨线灵敏度的探测器耦合 Monte Carlo 估计}

\author[inst1]{作者信息暂隐}
\address[inst1]{单位信息暂隐}

\begin{abstract}
银河系 511 keV 线是电子--正电子湮灭的特征谱学信号，但气球高度的指向式测量受大气伽马射线、粒子诱发仪器本底和延迟放射化限制。本文给出一种气球载望远镜的探测器耦合 Monte Carlo 估计，该望远镜由 10 m Ge(111) Laue 透镜和过渡边缘传感器（transition-edge sensor, TES）微量热计焦平面组成。聚焦源光子、瞬发大气粒子和光子、以及延迟放射性衰变在同一探测器/低温系统代理质量模型中输运，并采用同一事例定义，包括 CsI 反符合选择、拓扑/视场一致性检验和 $510.58$--$511.42\keV$ 线窗。延迟衰变从活化输运记录的放射性核素产生坐标采样。对参考流量为 $10^{-4}\phcms$ 的单色点源，第 15 天选后本底率和信号率分别为 $3.92\times10^{-2}\cps$ 和 $1.19\times10^{-3}\cps$，其中瞬发大气事例约占选后本底的 93\%。在采用的合成 20 d 轨迹和连续在源曝光假设下，统计计数指标给出 $Z_{20\mathrm{d}}=7.80$、净 $3\sigma$ 在源曝光 2.51 d，以及 20 d 的 $3\sigma$ 流量阈值 $3.85\times10^{-5}\phcms$。这些数值是针对未分辨线和当前参考质量模型的统计灵敏度；尚未包含全载荷瞬发本底模型、弥散天空前景，以及大气场、探测器响应、活化计算和本底估计中的系统不确定度。
\end{abstract}
...
% - 产生位置采样的延迟活化源通过与聚焦信号相同的线窗选择输运。
% - 参考未分辨线计算给出 F_3sigma(20 d)=3.85e-5 ph cm^-2 s^-1。

\section{引言}
\label{sec:intro}

511 keV 伽马射线是银河系电子--正电子湮灭的特征谱学信号。正电子产生后在星际介质中减速，随后可与电子直接湮灭并产生两条约 511 keV 的光子，也可先形成正电子素，再产生 511 keV 线和低于该能量的三光子连续谱。因此，该辐射复合结构同时探测正电子的产生、传播、热化和湮灭环境 \cite{Prantzos2011,Jean2006}。这一谱线最早由气球实验在银河中心方向报告，随后由高分辨 Ge 探测器明确证认为正电子湮灭辐射 \cite{Prantzos2011}。观测到的银河系流量对应约 $10^{43}\,\mathrm{e^+\,s^{-1}}$ 的正电子湮灭率，使其来源问题长期留在 MeV 伽马天文学中 \cite{Prantzos2011,Kierans2020}。

未分辨来源族不能只靠候选源清单判断。可能通道包括恒星爆发和新星中放射性核素的 $\beta^+$ 衰变、致密天体中的成对产生过程、宇宙线相互作用，以及可能的非标准粒子通道 \cite{Prantzos2011,Siegert2016AA,Yoneda2025}。观测到的湮灭形态还取决于正电子在热化和湮灭前的传播。谱学测量可通过线心、线宽和正电子素比例约束湮灭介质；511 keV 以上连续谱上限也可限制正电子注入能量 \cite{Prantzos2011,Jean2006,Siegert2019Kinematics}。更直接区分来源情形的观测量，包括紧凑源成分、小尺度结构、速度位移和空间分辨线宽，仍然受限于仪器灵敏度、角分辨率和本底系统误差。
...
宽视场康普顿望远镜仍是绘制 511 keV 大尺度天空图像的主要工具。与其互补的策略，是使用指向式望远镜把 511 keV 光子集中到小尺寸、高能量分辨率焦平面上。这样的系统检验的是更窄的问题：某个已知方向是否包含来自未分辨源、明亮中心核，或瞬变成对等离子体事件的紧凑线辐射。INTEGRAL 对银河系紧凑天体的 511 keV 上限给出了相关流量尺度；V404 Cygni 在 2015 年爆发期间提出的湮灭解释也说明，指向式致密天体随访与更宽泛的正电子来源问题有关，但需要结合独立宽带分析谨慎解释 \cite{DeCesare2011,Siegert2016V404,Roques2015}。因此，聚焦载荷并不是要取代宽视场康普顿任务，而是在较小视场内追踪另一类观测量。
...
详细 Monte Carlo 质量模型是解决这类问题的标准路线。INTEGRAL/\allowbreak SPI 响应矩阵生成需要高保真质量模型、地面和在轨标定比较，并在计算与存储代价可控的条件下分离成像响应和能量再分布响应 \cite{Sturner2003}。现代 MeV 任务研究，包括 AMEGO 和 COSI，也依赖 Geant4/MEGAlib 体系计算仪器响应、本底、事例重建和灵敏度 \cite{Agostinelli2003,Allison2016,Zoglauer2006,Caputo2017,Karwin2023,Gallego2025Balloon,Gallego2025,Beechert2022,Sleator2019,Ciabattoni2025ACS}。本文采用同样思路，但目标更窄也更苛刻：带 TES 焦平面的气球载、聚焦式、窄线 511 keV 望远镜。由于该研究以仿真为基础，本文不把输运代码本身当作证据；率级结果由明确的几何和源归一化、真实追踪焦面穿越记录、活化产生位置记录、统一探测器层选择，以及下游检查共同支撑。

本文在选后率层面评估该载荷。模拟链从 511 keV Ge(111) Laue 透镜源模型出发，将聚焦光子经探测器入射窗输运到工程化探测器/低温系统质量模型中。模型包括 TES 焦平面、多级热窗、Be 入射窗、Cu/W 被动屏蔽、低温结构代理质量以及 CsI 主动屏蔽。本底链路包括瞬发大气成分、活化积累、产生位置采样延迟源输运、主动屏蔽 veto、拓扑/视场一致性筛选、窄 511 keV 线窗、任务时间率演化和计数显著度估计。本文给出的是信号和本底通过同一探测器耦合选择定义后的选后率计算，而不是孤立有效面积估计。
...
本文考虑的仪器针对指向式窄线测量，而不是全天巡天成像。主要科学目标是在长时气球曝光中探测或限制 $10^{-4}\phcms$ 量级及以下的紧凑 511 keV 光子辐射。该流量尺度在本文中作为明亮中心核、紧凑源和瞬变成对等离子体情形的设计目标，并与宽视场康普顿巡天形成互补。
...
由此得到四条需求。第一，望远镜必须集中 511 keV 光子，使小焦平面不牺牲收集面积。第二，焦平面响应必须支持亚 keV 分析线窗，使线窗本底随窄谱窗而非宽 MeV 连续谱缩放。第三，探测器和屏蔽模型必须抑制大气、活化和侧向入射本底，同时显式计入 TES 吸收体附近的被动材料。第四，模拟链必须一致处理瞬发本底、延迟活化、聚焦信号、反符合 veto、事例拓扑和任务时间变化。表~\ref{tab:requirements_traceability} 给出紧凑可追溯摘要；详细 workflow 表不放在正文中。

\begin{table}[t]
\centering
\caption{参考未分辨线计算的紧凑可追溯摘要。}
...
科学驱动 & 模型需求 & 当前处理 \\
\midrule
紧凑 511 keV 目标 & 将光子集中到小探测器上 & 10 m Ge(111) Laue 光学，追踪焦面光子并在探测器模型中重放 \\
未分辨线谱学 & 保留窄线响应和光子流量归一化 & $510.58$--$511.42\keV$ 事例能窗；选后有效面积由同一探测器选择给出 \\
气球本底 & 对信号、瞬发和延迟流采用共同选择 & CsI 反符合、拓扑/视场一致性，以及产生位置采样延迟衰变 \\
参考曝光灵敏度 & 将选后率折叠到采用的曝光模型 & 合成 20 d 轨迹和连续在源曝光；不含目标星历或 duty-cycle 损失 \\
\bottomrule
\end{tabular}
\end{table}

这些需求也限定了结果解释。本文报告的阈值是未分辨线的参考曝光统计灵敏度，因为它只依赖能窗选择、反符合 veto、拓扑/视场一致性和任务时间率折叠。聚焦光斑和多环次级分析用来检验空间分析边界；在形成最终空间--谱似然并对本底扰动参数做剖面化处理之前，它们不替代线窗需求。

\section{模拟流程}
\label{sec:workflow}

图~\ref{fig:workflow} 总结了本文工作流。计算分为几何定义、瞬发与活化积累输运、延迟源构建、聚焦光学输运、探测器耦合信号重放、探测器事例筛选、任务时间率折叠、源情形归一化以及累积显著度评估。本文以产生位置采样延迟衰变作为主要延迟源表示：延迟衰变从记录到的放射性产生位置采样，而不是重新分配到轴对称径向分布。
...
主要率级计算结合瞬发和活化积累输运、固定第 15 天放射性核素群体、记录产生位置上的等活度点源采样点，以及下游探测器响应、事例选择和显著度计算。

每一层都有明确输入、物理或统计操作、传递给后续层级的输出，以及解释边界。几何定义固定质量模型和坐标策略，但不产生率。瞬发输运和活化输运分别定义直接大气本底与同位素产生历史。延迟源层把这些产生历史转换为固定第 15 天放射性核素群体，并输运采样的产生位置衰变。光学 Monte Carlo 只负责信号：它提供 Laue 透镜有效面积和真实追踪的聚焦相空间；光学-探测器耦合与探测器响应层再把该相空间转化为探测器耦合点源响应。任务时间折叠和累积计数随后把选后率转换为参考曝光统计灵敏度。

下文数值引用遵循同样的层级。第 15 天选后率是在聚焦信号流按 10 m
Ge(111) 有效面积和参考大气透过率缩放后引用；任务平均率来自时间相
关折叠；主要 $Z$、$T_{3\sigma}$ 和流量阈值来自累积显著度计算。
产生位置抽样检查只在采样源完成输运并通过同一探测器
...
探测器模型是一个工程化 Geant4/MEGAlib 代理质量模型，用来表示安装在小型稀释制冷机（DR）冷端的气球载 TES 焦平面组件 \cite{Shirazi2023,Gottardi2021TESReview}。它不是最终飞行 CAD，也不试图完整复现微加工 TES 膜层、布线、读出布局或全部低温服务部件。它也不包含完整吊舱、光学平台、透镜支撑结构、电子学、压力容器或弥散天体场。模型保留的是对 511 keV 透射、散射、活化、主动屏蔽 veto 和事例拓扑一阶敏感的探测器/低温系统材料柱和局部孔径。基准探测器/低温系统几何因此包含 TES 吸收体堆栈、衬底和窗口代理、Be 侧入射真空窗、被动屏蔽/准直材料、低温服务质量代理，以及分段 CsI 主动反符合屏蔽。图~\ref{fig:mass_model} 给出本文使用的二维质量模型示意图，表~\ref{tab:geometry} 列出进入率级计算前固定的探测器和分析参数。
...
TES 焦平面用 $\gamma$ 射线吸收体阵列加参数化量热响应表示，而不是完整复现微加工 TES/readout 结构 \cite{Zhang2022TESGamma}。在当前探测器概念中，TES 温度计膜是约 $200\,\mathrm{nm}$ 厚的 AlMn 薄膜，而 $\gamma$ 射线吸收体是 mm 量级金属结构。本文的非热探测器本底模拟中，511 keV 阻止、康普顿散射和活化主要由吸收体及其邻近支撑质量决定。显式敏感体积为 6 层、每层 376 个 Ta 吸收体像素，总计 2256 个像素。每个像素在侧入射坐标中为 $1.5\times1.5\times3.0\,\mathrm{mm^3}$，其中 $3.0\,\mathrm{mm}$ 深度沿聚焦束方向。TES 膜层、布线和读出细节通过假设能量响应和服务质量代理进入模型，而不是作为显式相互作用体积逐层建出。选后率模型对重建求和 TES 能量采用单一 Gaussian 事例能量响应代理，$\sigma=0.14\keV$，不模拟独立逐像素展宽。增益漂移、非 Gaussian 尾、堆积、饱和和响应参数扫描未包含在内，因此本文阈值应理解为该理想化响应包络内的结果。分析线窗另行固定为 $\wii=510.58$--$511.42\keV$。探测效率在本质量模型段不作为独立标量强加，而是由 Geant4 穿过吸收体/屏蔽几何的输运，以及后续事例选择和信号归一化链路共同给出。
...
TES 能量响应代理 & 单一 Gaussian 事例能量代理，$\sigma=0.14\keV$；不模拟独立逐像素展宽 \\
...
信号模型采用标准 Laue 透镜思路：入射 $\gamma$ 射线在 mosaic 晶体中以透射 Laue 几何发生 Bragg 衍射，并被集中到小面积焦平面探测器上 \cite{Barriere2009Laue,Barriere2011LauePrototype}。早期聚焦式 $\gamma$ 射线望远镜研究已经指出，探测器响应需要和聚焦相空间一起评估，而不能只用孤立的透镜有效面积代替 \cite{Weidenspointner2006MAX}。针对本文的 511 keV TES 概念 \cite{Shirazi2023}，基准光学构型为单能 Ge(111) Laue 环：焦距 $10\,\mathrm{m}$，25 个方形晶片，晶片边长 18 mm，mosaic FWHM 为 30 arcsec，有效 Ge 质量 $0.441\,\mathrm{kg}$。Ge(111) 衍射概率和角响应由外部 XOP/CRYSTAL rocking-curve map 给出 \cite{SanchezDelRio2011XOP}。有效面积归一化定义为环几何面积乘以模拟得到的、能够经探测器入口孔径出射的衍射光子比例，得到 $\aeff(511\keV)=20.1\,\mathrm{cm^2}$。
...
\caption{511 keV Laue 信号源的模拟导出径向--$z$ 视图。红色衍射扇束终止于真实追踪且满足 Be 入射孔径门的焦面穿越点。}
...
\caption{通过 Be 入射孔径筛选、并用于后续探测器重放的真实追踪焦面横向位置 $(x_{\rm fp},y_{\rm fp})$。红色圆为 Be 窗孔径。}
...
为了检验这一边界能否在需要时由同一 Geant4/MEGAlib 本底链处理，本文还运行了一个辅助的上游硬件计算。该计算在 10 m 上游透镜位置放置一个与 Laue 有效晶体等体积、等质量的 Ge 代理体。该代理体对应 25 个方形 Ge 晶片，边长 18 mm、厚度约 10.2 mm，总有效 Ge 体积为 $82.8\,\mathrm{cm^3}$，质量为 $0.441\,\mathrm{kg}$。大气远场源面由探测器加上游透镜的组合包络确定：10 m 透镜中心距离加有效晶体体积包络给出约 $1009\,\mathrm{cm}$，输运源面半径向上取整为 $1060\,\mathrm{cm}$，世界体半尺寸为 $2500\,\mathrm{cm}$。该质量代理使用与探测器本底链相同的八类 EXPACS/PARMA 粒子族（$\gamma$、$n$、$e^\pm$、$p$、$\alpha$ 和 $\mu^\pm$）完成瞬发输运和活化产生输运。该计算定义了上游质量代理的全粒子输运和同位素产生边界。有效 Ge 代理体产生的延迟成分随后从组合几何同位素产生记录中隔离出来，并按记录产生位置转换为衰变源，在同一探测器层选择下重放；其选后率结果在第~\ref{subsec:upstream_optics_background} 节给出。

对于聚焦点源响应，$\fzero=10^{-4}\phcms$ 表示参考入射 511 keV 光子流量。线窗信号响应 $1.19\times10^{-3}\cps$ 指该流量下经过探测器输运、参考大气透过因子和探测器层事例选择后的 $\wii$ 选后信号率。相应选后有效面积为 $A_{\mathrm{sel}}=S_{\wii}/\fzero=11.9\,\mathrm{cm^2}$。该参考归一化应理解为基准指向情形，而不是所有气球指向通用的大气透过率。低仰角或长斜程观测，包括北半球中纬度气球轨迹上的银河中心观测，应使用斜程大气透过模型；这类斜程计算仍是边界检查，不替代本文给出的参考曝光结果。
...
积分源面通量约为 $4.80$、$0.462$、$0.199$、$0.117$、$0.112$、$0.0115$、$0.0050$ 和 $0.0056\,\mathrm{cm^{-2}\,s^{-1}}$，分别对应 $\gamma$、$n$、$e^-$、$e^+$、$p$、$\alpha$、$\mu^-$ 和 $\mu^+$。非光子成分合计约为 $0.91\,\mathrm{cm^{-2}\,s^{-1}}$，约占全空间源面积分通量的 16\%。如果完全按流量比例分配模拟计数，这些低通量粒子的探测器命中和活化统计会较差；因此非光子粒子族采用复制输运样本提高 Monte Carlo 统计，再在率归一化时去权重。所有这些粒子随后通过 Geant4/MEGAlib 在探测器/低温系统质量模型中输运 \cite{Agostinelli2003,Zoglauer2006}。瞬发流和活化产生流各生成约 $2.52\times10^7$ 个入射一次粒子。每个粒子种类用自身输运曝光归一化：
\begin{equation}
...
\end{equation}
其中 $R_k$ 是由活化产生输运推得的产生率，参考第 15 天状态取 $t_{\mathrm{flight}}=15\,\mathrm{d}$。参考延迟源构建使用的固定延迟总活度为 $85.45\,\mathrm{Bq}$。

位置信息并不是只来自聚合后的同位素存储文件。标准同位素存储可记录产生体积和核素计数；产生位置采样构建还在 Geant4/MEGAlib 的 stepping-action 层加入了自定义记录钩子。在活化积累模式中，每当一个延迟核素被存入同位素清单时，该钩子在同一位置写出产生位置记录，包括逻辑体名称、pre-step 位置 $(x,y,z)$、核素编号 $1000Z+A$、激发能、时间、过程以及 track ancestry 信息。固定第 15 天放射性群体随后按体积、核素和激发态与这些位置记录匹配。因此，延迟源可使用模拟产生坐标，而不是只有体积积分活度。

图~\ref{fig:delayed_position_distribution} 给出放射性产生位置与固定第 15 天活度群体匹配后的延迟源位置分布。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{paper_source_figure_table/fig_delayed_position_rpip_distribution.png}
\caption{由记录的放射性核素产生位置构建的产生位置采样延迟源分布。左：记录产生位置在 X--Z 平面上的活度加权分布，并叠加探测器/低温系统代理几何。中上：同一分布的 X--Y 活度加权足迹，图中给出 Be 孔径和屏蔽包络作为尺度参考。右侧和下方：按固定第 15 天活度加权后的主要产生体积和主要核素。匹配后的产生位置表中活度权重总和为 $85.45\,\mathrm{Bq}$。}
...
是否需要保留产生位置信息，也可以直接用同一张产生位置表检验。延迟衰变是在局部材料中发射的，因此下游衰减、主动屏蔽 veto 概率和事例拓扑都取决于核素产生位置。带电强子和缪子相对中子有不同的能量损失、停止和俘获历史，因此它们产生的活化核素不应被假定为共享同一个材料或坐标分布。为量化这一点，我们按入射粒子族和材料类别对加权产生行分组。对两个入射粒子族 $a$ 和 $b$，使用活度归一化材料分布的 total-variation 距离
\begin{equation}
  D_{\mathrm{TV}}(a,b) =
...
图~\ref{fig:position_sampling_necessity} 给出这一检验结果。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{paper_source_figure_table/fig_position_sampling_necessity.png}
\caption{产生位置采样诊断图。上排按入射粒子族比较固定第 15 天加权产生表：材料占比、核素组成和总活度。下排给出中子、质子和 $\mu^-$ 诱发产物的 X--Z 记录产生位置图。不同入射粒子族的分布并不可互换：中子诱发活度主要在 CsI 和 $^{128}$I 中，$\mu^-$ 诱发活度则集中在含 Al/Cu/W 的服务和屏蔽材料中，并以 Mg/Na/Ne 产物为主。}
\label{fig:position_sampling_necessity}
\end{figure}

在固定第 15 天表中，中子诱发产物贡献约 $82.5\,\mathrm{Bq}$，$\mu^-$ 诱发产物贡献 $2.75\,\mathrm{Bq}$，质子诱发产物贡献 $0.15\,\mathrm{Bq}$，$\alpha$ 诱发产物贡献 $0.022\,\mathrm{Bq}$，另有更小的 $\mu^+$ 和 $e^+$ 贡献。中子诱发活度仍主要产生在 CsI 主动屏蔽中；$\mu^-$ 成分则明显不同，其中 Cu/支撑材料、Al 壳层和含 W 被动材料分别占 $\mu^-$ 诱发活度的 32\%、29\% 和 21\%。因此，体积积分源或轴对称径向源可以保持总活度，但会抹掉入射粒子族、材料、核素和坐标之间的相关性。产生位置构造在进入探测器输运之前保留了这些相关性。

延迟衰变源随后从采样的产生位置发射，而不是从轴对称径向分布发射。设加权产生表中第 $j$ 行的活度权重为 $w_j$，总活度为 $A=\sum_j w_j$。产生位置近似以 $p_j=w_j/A$ 为概率有放回抽取 $M$ 个产生位置，并给每个采样点源相同通量 $A/M$。因此第 $j$ 行获得的期望活度为
\begin{equation}
  \mathbb{E}\!\left[N_j\frac{A}{M}\right] = M p_j \frac{A}{M} = w_j ,
\end{equation}
所以该估计保持总活度，并且对加权空间分布无偏。这里的 $M$ 表示用于近似加权产生位置分布的等活度点源采样点数；它是采样规模，不是物理放射性产生行的数量。参考延迟源在源定义层面保持总固定活度，并在与瞬发流相同的探测器/低温系统几何中重放。名义延迟计算输运 $10^6$ 个延迟衰变；相应 Cosima live time 是生成衰变序列的输运记录，不作为物理气球曝光时间使用。

表~\ref{tab:background_source_model} 汇总了当前探测器耦合计算中的源层级。它放在源模型部分，是因为这里定义的是入射粒子场和延迟放射性群体；探测器 veto 和拓扑选择只在后续选择部分施加。
...
该产生位置表示不是对每条产生行都建立一个采样点源的极限源，而是统计采样表示。源清单检查用于核对活度守恒和源序列化；选后率行为则在探测器输运和事例选择之后另行检验。
...
\end{equation}
若 $E_{\mathrm{TES}}^{(c)}\in\wii$，该候选组进入线窗；若
\begin{equation}
...
第~\ref{sec:mission_time} 节定义的任务时间折叠。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{paper_source_figure_table/fig_selection_veto_time_axis.png}
\caption{参考计算的探测器选择和任务时间折叠诊断。
A 面板给出 $\wii$ 中瞬发本底、延迟活化以及按 $\fzero=10^{-4}\phcms$
缩放的聚焦信号在预 veto、主动屏蔽通过和拓扑/视场选后三个阶段的率。
B 面板给出相应保留比例。C 面板给出主动屏蔽通过后的单点、已重建多点和保留未重建类别。
D 面板给出参考点源情形的任务时间率折叠，并显示解析
偶然符合活时间因子。}
...
实现检查在中心点和两个海拔极值点重新计算了轨迹公式。第 15 天锚点处参考大气透过率为 $T=0.739$，高、低海拔探针对应 $T=0.811$ 和 $0.646$。用第 15 天率和这些缩放重构 $\wii$ 瞬发、延迟、本底和信号率，可复现任务时间折叠摘要。另一个使用相同轨迹探针处本地 PARMA 谱的粒子族重权重检查显示，归一化核素--体积活度分布的 total-variation 距离变化小于 $6\times10^{-3}$。这支持在本文小范围轨迹内，于入射粒子族重权重层面使用标量延迟产生率/活度折叠。它不是分布不变性的完整物理证明：同一粒子种类内部的能量/角度相关活化截面变化和输运位置漂移，仍需要在低海拔、参考点和高海拔分别运行专门的活化产生输运来直接检验。
...
\end{equation}
缩放。线窗结果和次级比较均一致使用这一 Gaussian counting 约定，但它只是诊断性选后率指标，不是最终飞行发现似然。对本文 20 d 参考情形，信号/本底比约为 3\%；仅 1\% 的 Gaussian 本底归一化不确定度就会把近似显著度从约 7.8 降至约 2.8。因此，飞行分析需要离源或控制能带本底约束，并对扰动参数做剖面化处理。
对主要 20 d 线窗结果，任务折叠后的计数约为
$6.8\times10^4$ 个本底计数和 $2.0\times10^3$ 个 $\fzero$ 信号计数，
因此这一近似只用于高计数选后率情形。上游 Ge 代理分量的零选出事例
结果则单独用 Poisson 零计数上限报告。
探测器选择摘要还记录第 15 天直接期望。本文主要结果不采用该
常率直接值，而使用任务时间折叠和最终累积显著度。
对当前线窗而言二者差异不大，但明确区分这两类量是比较不同分析配置
的必要条件。

\section{结果}
\label{sec:results}

\subsection{主要未分辨线灵敏度}
\label{subsec:primary}

表~\ref{tab:primary_sensitivity} 给出主要未分辨线结果。对 $\fzero=10^{-4}\phcms$，第 15 天选后率为
\begin{equation}
...
\end{equation}
对应 20 d 诊断计数显著度为 $Z_{20\mathrm{d}}=7.80$，净 $3\sigma$ 在源曝光为 $2.51\,\mathrm{d}$。20 d $3\sigma$ 流量阈值为 $3.85\times10^{-5}\phcms$。

\begin{table}[t]
\centering
\caption{主要未分辨线选后率结果和本底组成。}
...
量 & 数值 & 解释 & 注释 \\
\midrule
第 15 天选后本底 & $3.92\times10^{-2}\cps$ & 瞬发 + 延迟 & 探测器层选择 \\
第 15 天 $\fzero$ 下选后信号 & $1.19\times10^{-3}\cps$ & 聚焦点源 & $\fzero=10^{-4}\phcms$ \\
瞬发本底成分 & $3.66\times10^{-2}\cps$ & 选后本底的 93.4\% & 主导成分 \\
延迟活化成分 & $2.6(5)\times10^{-3}\cps$ & 选后本底的 6.6\% & 全统计 authority；独立收敛 screen 给出 $2.21(22)\times10^{-3}\cps$ \\
20 d 诊断 $Z$ & 7.80 & 计数指标 & 不含本底扰动参数 \\
净 $3\sigma$ 曝光 & 2.51 d & 参考曝光 & 不是经过的飞行时间 \\
$\fthree(20\,\mathrm{d})$ & $3.85\times10^{-5}\phcms$ & 未分辨线阈值 & 仅针对参考模型 \\
\bottomrule
\end{tabular}
}
\vspace{0.5ex}
...
\footnotesize
本底率和信号率为第 15 天选后率。显著度、净曝光和流量阈值使用任务时间折叠。延迟行保留主要闭合计算使用的单一源采样全统计 authority。独立的四源采样选后率 screen 从 103 个选后延迟事例得到 $2.21(22)\times10^{-3}\cps$，与该 authority 值在采样不确定度内一致；这里仍不包含源模型和活化系统误差。
\end{minipage}
\end{table}

线窗结果使用窄能窗、主动屏蔽 veto、拓扑/视场一致性选择和任务时间折叠。空间门限、45$^\circ$ 视线折叠和固定模板多环似然不用于改善本文给出的阈值。

\subsection{本底组成}
\label{subsec:bgcomp}

选后 $\wii$ 本底由瞬发大气事例主导。表~\ref{tab:primary_sensitivity} 中的选后流分解对应 93.4\% 瞬发和 6.6\% 延迟活化。延迟成分虽然率较小，但其源分布具有空间和材料结构。当前主要延迟 authority 已由独立选后率收敛 screen 支持，而不只依赖源清单检查。

\subsection{延迟源归一化和选后率收敛检查}
\label{subsec:convergence}

参考延迟计算附带的源构建审计记录了主要率级结果所使用的延迟源归一化。

固定源包含 251,681 条加权产生位置行，并在 NUBASE 基态修正后保持 $85.45\,\mathrm{Bq}$ 的总延迟活度。该修正前的原始延迟活度为 $87.48\,\mathrm{Bq}$。源序列化对活度的保持优于 $10^{-9}$ 相对精度。这些检查验证源归一化和记账；延迟选后率必须在通过探测器几何和共同事例选择输运之后再检验。

因此本文进行了一个最低选后率收敛 screen，包含四个相互独立的产生位置源采样。每个源采样均通过同一探测器/低温系统几何输运 $10^6$ 个延迟衰变，并使用指向同一参考几何的 source card 和 SIM header。四个运行总共产生 103 个选后延迟事例，合并选后率为 $2.21\times10^{-3}\cps$，Poisson 统计不确定度为 $0.22\times10^{-3}\cps$（相对不确定度 9.85\%）。逐运行比较通过 $2\sigma$ 一致性 screen；相对合并率的最大偏差为 1.82 个标准差。这一结果闭合了本文延迟成分所需的最低选后率收敛 screen，但活化截面、大气场系统误差以及详细源/配置 provenance 仍属于单独的可复现性不确定度。
...
$6.38\times10^{-5}\cps$。该上限约为选后 $\wii$ 本底率
$3.92\times10^{-2}\cps$ 的 $0.16\%$，因此隔离的 Ge 代理延迟分量在当前精度
下不改变本文给出的线窗灵敏度。上游硬件的瞬发自本底仍不并入主要
...
主要结果是，参考产生位置延迟源计算对所建模的聚焦式 TES 线谱仪，在未分辨线情形下给出 $3.85\times10^{-5}\phcms$ 的 20 d 诊断 $3\sigma$ 点源阈值。该阈值没有把聚焦光斑空间分析作为主要结果使用，而是来自光学集中、亚 keV 能窗以及探测器耦合 veto/拓扑选择三者的组合。

该计算应作为指向式、窄线、紧凑源估计来和现有 511 keV 仪器比较，而不是作为全天成像预测。INTEGRAL/SPI 和 COSI 提供弥散 511 keV 天图的参照测量 \cite{Siegert2016AA,Kierans2020,Siegert2020COSI,Yoneda2025}。COSI 的同行评议标定和模拟研究为探测器响应、事例重建和气球本底验证提供更直接的技术比较 \cite{Beechert2022,Sleator2019,Gallego2025Balloon,Ciabattoni2025ACS}。本文考虑的 TES Laue 透镜概念与其互补：它以天空覆盖换取集中束流和亚 keV 线窗。因此，相关比较对象是紧凑 511 keV 目标、中心核搜索和已知或受限位置瞬变的后随观测。相同的聚焦孔径不能替代完整弥散核球和银盘光度测量，因为它只采样很小的立体角。

产生位置延迟源处理针对的是径向分布压缩中的一个具体近似。虽然在当前计算中延迟活化仅占选后线窗本底约 6.6\%，但它具有空间和材料结构。保留采样产生位置可避免人为方位角或径向重分布成为未量化系统误差。延迟成分已经通过包含四个独立产生位置采样的最低选后率收敛 screen，但这不应被理解为完整的延迟本底系统误差包络。

本底组成指出了首先应检验的工程敏感项，而不是已经完成的优化结论。选后线窗率仍由瞬发大气流主导，因此屏蔽几何、侧入射准直、焦平面附近材料、拓扑选择和多像素事例重建是当前本底最直接的控制项。延迟活化较小但仍具有空间和材料结构，因此率级计算保留产生位置分布。CsI 屏蔽中碘活化占优也提示后续应进行屏蔽材料 trade study。
...
更一般地，表中 $3\sigma$ 流量阈值是基于参考源场、质量模型和探测器响应假设的统计选后率阈值；它们尚未包含大气通量归一化、活化截面或物理列表选择、材料和几何公差、探测器能量响应、veto 阈值标定、重建效率系统误差或本底估计扰动参数的系统误差包络。投稿级飞行灵敏度应以可剖面化本底扰动参数的空间--谱似然取代诊断性 $S/\sqrt{B}$ 指标 \cite{Cowan2011}。
...
$r_{90}$ 和多环似然等空间分析可作为诊断交叉检查，但尚不是最终扰动参数剖面似然。因此，本文给出的灵敏度仍是线窗选后率结果，而不是经过空间似然优化后的结果。
...
本文给出了气球载指向式 511 keV TES Laue 透镜望远镜的探测器耦合本底和灵敏度研究。参考计算把 Ge(111) Laue 光学响应与 TES 焦平面质量模型、瞬发大气输运、延迟活化、主动屏蔽 veto、拓扑/视场选择、任务时间率折叠和计数显著度评估结合起来。在主要 $510.58$--$511.42\keV$ 未分辨线窗中，第 15 天选后率为 $B\simeq3.92\times10^{-2}\cps$，对 $10^{-4}\phcms$ 参考点源有 $S\simeq1.19\times10^{-3}\cps$；任务时间折叠后得到 $Z_{20\mathrm{d}}\simeq7.80$ 和 $\fthree(20\mathrm{d})\simeq3.85\times10^{-5}\phcms$。该结果为继续评估聚焦式 TES 谱学作为 511 keV 紧凑源和中心核搜索的互补技术提供了探测器耦合选后率依据，同时指出瞬发大气本底抑制、延迟活化系统误差验证和重建验证是下一阶段的主要验证任务。
