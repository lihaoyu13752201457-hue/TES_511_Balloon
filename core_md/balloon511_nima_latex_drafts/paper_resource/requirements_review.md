...
Status: draft review for Section 2 resources.
Date: 2026-06-14.
...
- The numerical values used in the traceability table match the current exact-position authority:
  - `A_eff(511)=20.08476 cm2`;
  - `W2=510.58-511.42 keV`;
  - Step05 `B/S=0.0624651/0.00118117 cps`;
  - Step08 `Z20d=6.15522`, `F_3sigma(20d)=4.87391e-5 ph cm^-2 s^-1`;
...
- The SVG figure is editable line art. It is not yet included in the LaTeX source because `pdflatex` cannot include SVG directly without conversion.
...
- Exact-position delayed-source convergence now supports using `fullstat_v2_exactpos` as the rate authority for W2; a one-block-per-RPIP stress test remains an engineering upgrade, not a blocker for this requirement narrative.
...
- PDF regeneration is verified for this draft state. The system TeX tree still does not have an installed `elsarticle.cls`, so the build used a temporary CTAN copy generated under `/tmp/elsarticle_ctan/elsarticle` and supplied through `TEXINPUTS`. Both English and Chinese drafts compiled successfully to PDF from the current `.tex` sources.
...
- A structured JSON check against the exact-position closure summary passed for the resource values `0.0624651`, `0.00118117`, `11.8117`, `6.15522`, `4.87391`, `0.0590827`, and `0.00338234`.
- Full LaTeX compilation passed with `TEXINPUTS=/tmp/elsarticle_ctan/elsarticle//:`:
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error balloon511_nima_draft_en.tex`;
  - `latexmk -xelatex -interaction=nonstopmode -halt-on-error balloon511_nima_draft_zh.tex`.
- Current render outputs: English draft PDF has 16 pages; Chinese draft PDF has 15 pages.
