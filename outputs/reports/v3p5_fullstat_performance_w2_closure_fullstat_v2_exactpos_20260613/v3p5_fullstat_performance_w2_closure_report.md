# v3p5 Exact-Position Performance and W2 Background Closure
...
- Full-stat Step02 prompt and buildup transport each generated `25210216` / `25210216` particles; delayed transport stored `SE=1000000` over `TE=11530.5 s`.
- W2 final background is `0.0624651 cps` with signal `0.00118117 cps` at `1e-4 ph cm^-2 s^-1`; the mission-mean Step06 fold is `0.0626835` / `0.00117281 cps`.
- The Step08 time-dependent W2 result is `Z20d=6.15522`, `T3=4.73758 day`, and `F_3sigma(20d)=4.873910e-05 ph cm^-2 s^-1`.
- The 1 Ms comparison gives v3p5 W2 `F_3sigma=6.325642e-05 ph cm^-2 s^-1`; external markers are included for 511-CAM, SPI, and COSI with source notes in the JSON.
...
| --- | ---: | --- |
| v3p5 W2 | 6.325642e-05 | v3p5_time_dependent_cumulative_interpolation |
...
The largest selected component is `prompt:eplus` with `80` selected events, `0.0543377 cps`, and `87.0%` of the W2 final background rate.
The delayed slice is small; within delayed-only W2 events, `Cu64` is the largest nuclide with `29` events and `74.4%` of delayed W2 events.
...
- Full-stat label is `fullstat_v2_exactpos`.
...
- Boundary sidecars are packaged at `outputs/reports/v3p5_boundary_closure_fullstat_v2_exactpos_20260613/v3p5_boundary_closure_report.md`.
...
- 45 deg LOS W2 sidecar: `Z20d=5.42468`, 20-day 3sigma flux `5.530275e-05` ph cm^-2 s^-1.
- 45 deg LOS `spot_r90` sidecar: `Z20d=7.20513`, 20-day 3sigma flux `4.163698e-05` ph cm^-2 s^-1.
- Fixed-template multi-annulus spatial-likelihood sidecar: `Z20d=8.45781`, 20-day 3sigma flux `3.547017e-05` ph cm^-2 s^-1.
- Exact-position delayed-source status: `PASS_EXACT_RPIP_POINTSOURCE_PRODUCTION_TRANSPORT`.
- Exact-position feasibility status: `EXACT_RPIP_POINTSOURCE_SMOKE_VALIDATED_AND_PROMOTED_TO_V3P5_RUN`.
...
- summary JSON: `outputs/reports/v3p5_fullstat_performance_w2_closure_fullstat_v2_exactpos_20260613/v3p5_fullstat_performance_w2_closure_summary.json`
- HTML report: `outputs/reports/v3p5_fullstat_performance_w2_closure_fullstat_v2_exactpos_20260613/report.html`
